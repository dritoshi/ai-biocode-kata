...
### 方針A: モデル名を本文から追い出す（推奨）

本書の CLAUDE.md は既に「本文では概念で記述する／ツール固有の操作は対照表で示す／特定ツールの名前をベタ書きしない」と定めている。**モデル名についても同じ方針を適用すれば、以後の改訂負担が構造的に下がる。**

- 本文（§0-7 の使い分け指針、コスト意識）は「最上位／バランス／軽量の3階層」という概念記述に徹する
- 具体的なモデル名・価格・ベンチマーク値は**付録Bの対照表1箇所に集約**し、冒頭に時点を明記する
- 現在は §0（48箇所）、はじめに（11箇所）、付録B（10箇所）、用語集（8箇所）、付録D（2箇所）に分散しており、改訂のたびに全箇所を追う必要がある
