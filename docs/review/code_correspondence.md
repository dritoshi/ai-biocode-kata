...
- 生成日: 2026-07-26
- 対象コミット: `2763e23073dd00e5a39b29ee91869c235661c3a7`
...
| E5 | 12 | 配置が必要だが対応実体がない |
| EN | 395 | 規約上、対応実体は不要 |
| **合計** | **529** | 全本文ブロック |

配置が必要なブロックは134件である。E5は12件であり、具体的な解消順序はE5解消計画に記録した。

`scripts/ch*` 側は全140ファイルで、本文コードと直接対応86件、本文から参照のみ25件、本文コードとの対応なし29件である。
...
| **合計** | **529** | **134** | **29** | **38** | **18** | **37** | **0** | **12** | **395** |

## 3. 対応実体がないブロック

| ID | 章・開始行 | 種別 | 見出し |
|---|---|---|---|
| `B-08-026` | `ch08` L631 | config_workflow | ワークフローYAMLの書き方 |
| `B-08-027` | `ch08` L684 | config_workflow | ワークフローの拡張 |
...
分類overrideは27件、関係overrideは107件である。override対象の本文ハッシュが変わった場合、生成処理は停止する。
