...
- 生成日: 2026-07-26
- 対象コミット: `e5c742ac83c729b547e77aa5cd4bee6fc5e8acae`
...
| E5 | 16 | 配置が必要だが対応実体がない |
| EN | 395 | 規約上、対応実体は不要 |
| **合計** | **529** | 全本文ブロック |

配置が必要なブロックは134件である。E5は16件であり、具体的な解消順序はE5解消計画に記録した。

`scripts/ch*` 側は全137ファイルで、本文コードと直接対応83件、本文から参照のみ25件、本文コードとの対応なし29件である。
...
| **合計** | **529** | **134** | **28** | **38** | **15** | **37** | **0** | **16** | **395** |
...
分類overrideは27件、関係overrideは103件である。override対象の本文ハッシュが変わった場合、生成処理は停止する。
