...
本書ではこれまで「ライブラリ」「パッケージ」「モジュール」「API」といった用語を断りなく使ってきた。[§5 開発環境の構築](./05_dev_environment.md)では `pip install` でパッケージをインストールし、[§10 データ処理ライブラリ](./10_data_processing.md)では `import numpy` でライブラリを読み込んだ。しかし、これらの用語が正確に何を指しているのか、`import` 文の裏で何が起きているのかは説明していない。この理解がなければ、`ModuleNotFoundError` や `ImportError` に遭遇したとき、原因がインストール漏れなのか、パスの問題なのか、環境の不一致なのかを切り分けられない。
...
本章は論理的には[§5 開発環境の構築](./05_dev_environment.md)の前に位置する内容だが、暫定的に§21として配置している。[§5 開発環境の構築](./05_dev_environment.md)以降を読み進めて「ライブラリとは何か」「importはどう動いているのか」といった疑問が生じたときに、本章に戻って参照してほしい。

本章では、プログラムの実行の仕組み（コンパイルとリンク）、Pythonのモジュールシステム、依存関係の問題、そしてAPIの概念を順に学ぶ。
...
[§2 ターミナルとシェルの基礎](./02_terminal.md#2-3-環境変数とパス)で学んだPATH環境変数は「実行ファイルの検索パス」だった。共有ライブラリにも同様の仕組みがある:
...
🧬 **condaと共有ライブラリ**: condaは、Pythonパッケージだけでなく、C/C++製ツールとその依存共有ライブラリもconda環境内にインストールする。biocondaチャネルからsamtoolsをインストールすると、libhtsやlibzもconda環境の `lib/` ディレクトリに配置される。これにより、システムのライブラリバージョンに依存しない。ただし、conda環境外のツールとの相互作用や、OSレベルのライブラリ（CUDA等）までは管理できない。完全な環境の隔離が必要な場合は、[§13 コンテナと再現性](./13_container.md)で学ぶコンテナを使う。
...
本書のサンプルコード `scripts/ch21/mylib/` がパッケージの実例である。ディレクトリ構造を見てみよう:
...
```python
# scripts/ch21/mylib/__init__.py
from scripts.ch21.mylib.core import gc_content, reverse_complement
from scripts.ch21.mylib.utils import validate_sequence
...
```python
# __init__.py がなければ:
from scripts.ch21.mylib.core import gc_content

# __init__.py のおかげで:
from scripts.ch21.mylib import gc_content
```
...
```python
from scripts.ch21.module_demo import show_sys_path, find_site_packages
...
仮想環境をアクティベートすると、その環境の site-packages が `sys.path` の先頭付近に追加される。これにより、プロジェクトごとに異なるバージョンのパッケージを使い分けられる。詳しくは[§5 開発環境の構築](./05_dev_environment.md#5-1-pythonの環境管理)を参照。

### PYTHONPATH環境変数

[§2 ターミナルとシェルの基礎](./02_terminal.md#2-3-環境変数とパス)で学んだPATH環境変数の考え方は、Pythonの `PYTHONPATH` にもそのまま当てはまる。PATHが「実行ファイルの検索パス」であるのに対し、PYTHONPATHは「Pythonモジュールの検索パス」である:
...
推奨されるのは、[§8 成果物の形式とプロジェクト設計](./08_deliverables.md)で学ぶ `pip install -e .`（開発モードインストール）である。`pyproject.toml` にパッケージ情報を記述し、`pip install -e .` でsite-packagesにリンクを作成すれば、PYTHONPATHを操作せずに自作モジュールをインポートできる。
...
2. **環境の分離**: プロジェクトごとに仮想環境を分ける（[§5 開発環境の構築](./05_dev_environment.md)）
3. **コンテナの利用**: OSレベルの依存まで含めて隔離する（[§13 コンテナと再現性](./13_container.md)）

### 環境隔離で解決できること・できないこと

[§5 開発環境の構築](./05_dev_environment.md)で学ぶvenvやcondaと、[§13 コンテナと再現性](./13_container.md)で学ぶコンテナでは、隔離できる範囲が異なる:
...
コマンド名、サブコマンド、オプション、引数の組み合わせが、CLIツールの「API」である。詳しくは[§9 CLIツールの設計](./09_cli.md)を参照。
...
URLの構造、HTTPメソッド、パラメータ、レスポンス形式が、Web APIの「API」である。詳しくは[§18 データベースとAPI](./18_database_api.md)で扱う。

### APIの変更がコードを壊す

エージェントが生成するコードは、ライブラリのAPIを使っている。ライブラリのバージョンが上がってAPIが変わると、以前は動いていたコードが壊れる。これが[§6 バージョン管理](./06_git.md#6-4-セマンティックバージョニング)で学ぶセマンティックバージョニングが重要な理由の一つである:
...
[§1 設計原則](./01_design.md#単一責任原則single-responsibility-principle-srp)ではSOLIDの最初の文字であるSRP（単一責任原則）を学んだ。SOLIDとは、Robert C. Martinが提唱した5つの設計原則の頭文字であり [5](https://web.archive.org/web/20150906155800/http://www.objectmentor.com/resources/articles/Principles_and_Patterns.pdf)、よいAPIやソフトウェア構造を設計するための指針である。ここでは残りの4原則の概要を紹介する。

**S — 単一責任原則**（Single Responsibility Principle; SRP）: [§1 設計原則](./01_design.md#単一責任原則single-responsibility-principle-srp)で学んだとおり、関数やクラスの変更理由は1つだけであるべきである。
...
| 概念 | 意味 | 本書での関連箇所 |
|------|------|-----------------|
| コンパイル・リンク | ソースコードを実行ファイルに変換する過程 | [§13 コンテナと再現性](./13_container.md)（共有ライブラリの問題） |
| 共有ライブラリ | 複数プログラムで共用される動的ライブラリ | [§5 開発環境の構築](./05_dev_environment.md)（conda管理） |
| モジュール | 1つの `.py` ファイル | [§8 成果物の形式とプロジェクト設計](./08_deliverables.md)（プロジェクト構造） |
| パッケージ | `__init__.py` を含むディレクトリ | [§8 成果物の形式とプロジェクト設計](./08_deliverables.md)（pip パッケージ） |
| `sys.path` | Pythonのモジュール検索パス | [§5 開発環境の構築](./05_dev_environment.md)（仮想環境） |
| 依存関係地獄 | バージョン要求の衝突 | [§5 開発環境の構築](./05_dev_environment.md)（環境隔離） |
| API | ソフトウェアの利用窓口 | [§9 CLIツールの設計](./09_cli.md)、[§18 データベースとAPI](./18_database_api.md) |
| SOLID原則 | よい設計の5原則 | [§1 設計原則](./01_design.md)（SRP） |
...
次に進む章として、環境管理の実践は[§5 開発環境の構築](./05_dev_environment.md)、プロジェクト構造の設計は[§8 成果物の形式とプロジェクト設計](./08_deliverables.md)、コンテナによる完全な隔離は[§13 コンテナと再現性](./13_container.md)を参照してほしい。
