...
SRPはオブジェクト指向設計の5原則**SOLID**の最初の文字（S）にあたる。残りの4原則（Open/Closed、Liskov Substitution、Interface Segregation、Dependency Inversion）は、よいAPIやインターフェースを設計するための指針であり、[§5 ソフトウェアの構成要素](./05_software_components.md#5-4-apiとインターフェース)で概要を学ぶ。初心者はまずSRPと関心の分離を確実に身につけることを優先してほしい。
...
| **CI/CD** | 継続的インテグレーション/デリバリー | コミットするたびにテストが自動実行される仕組み（[§8 テスト・品質管理](./08_testing.md)で詳述） |
...
これらの原則は、[§8 テスト・品質管理](./08_testing.md)で学ぶテストや、[§10 成果物の形式とプロジェクト設計](./08_project.md)で学ぶプロジェクト構造と密接に関連する。本章の原則を意識しながら以降の章に進んでほしい。
