...
- **Agans, D. J. *Debugging: The 9 Indispensable Rules for Finding Even the Most Elusive Software and Hardware Problems*. AMACOM, 2002.** — デバッグの9つの鉄則を提示する実践書。「Understand the System」「Make It Fail」「Quit Thinking and Look」など、本章で紹介したアプローチと同じ精神に基づく教科書である。
- **Zeller, A. *Why Programs Fail: A Guide to Systematic Debugging* (2nd ed.). Morgan Kaufmann, 2009.** — 科学的なデバッグ手法（デルタデバッギング、原因の系統的な絞り込み）の教科書。本章で扱った最小再現例の作成手法を理論的に裏付ける。やや学術的だが、デバッグを「勘」ではなく「方法論」として捉える視点が得られる。
