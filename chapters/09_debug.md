...
> — シロエ, 橙乃ままれ『ログ・ホライズン』第1巻 異世界のはじまり (エンターブレイン, 2011)
...
特に注意すべきは**サイレントバグ**——エラーは出ないが結果が間違っているケースである。座標変換のOff-by-oneエラー（[9-3節](#9-3-よくあるバグのパターン)参照）のように、「動くが1塩基ずれている」バグは、期待値を明確にしていなければ見逃してしまう。
...
後の[§11-3 ロギング](./11_cli.md#11-3-ロギング)で扱う `logging` モジュールを使えば、これらの問題を解決できる。ここではデバッグ局面に特化した使い方を先取りして紹介する。
...
後の[§11-3 ロギング](./11_cli.md#11-3-ロギング)では `logging` モジュールの基本設定を扱う。ここでは、デバッグ局面での応用パターンを先に紹介する。
...
2. 後の[§17-1](./17_performance.md)で紹介する `memory_profiler` で、メモリ使用量の推移を確認する
3. メモリの急増箇所が特定できたら、後の[§17-3](./17_performance.md)で扱うチャンク処理やジェネレータで対処する

新規コードを書く際は、データサイズの事前見積もり（レコード数 × 1レコードあたりのメモリ使用量）を行い、利用可能メモリと比較する習慣をつけよう。

### 並列処理のバグ

後の[§17-2](./17_performance.md)で並列処理を扱うが、並列処理固有のバグパターンも存在する。
