...
依存を増やしたくない配布物や、標準ライブラリだけで完結させたい環境では Argparse も十分に実務的な選択肢である。本書では説明の一貫性を優先して Click を標準とするが、Argparse を「古いので避けるべき」と理解してはいけない。
...
gc-filter -- -weird.fasta

# - 始まりの引数を複数渡す場合も -- で区切ればよい
seqtool convert --format fasta -- -1.fq -2.fq
```

argparse も click も、この `--` 慣習を標準で解釈する（getopt互換の挙動）。自分でCLIを実装するときに特別な対応は不要だが、「なぜ `-` 始まりのファイル名が読めないのか」と質問されたときに `--` を案内できると、ユーザーの助けになる。
...
|---|---|---|
| `0` | 正常終了 | 処理が成功した |
| `1` | 一般的なエラー | ファイルが読めない、解析に失敗した |
| `2` | コマンドの使い方の誤り | 不正なオプション、必須引数の欠落 |
...
> <a id="column-ch11-01"></a> 🤖 **コラム: フォールバックと仲間たち — 障害対応の4つの概念**
>
> AIエージェントは「フォールバックを用意しましょう」「フォールトトレラントにしましょう」と提案することがある。これらは似ているが異なる概念であり、混同するとレビュー時に議論が噛み合わなくなる。
>
> | 概念 | 何をするか | 方向 | 例 |
> |------|----------|------|-----|
> | フォールバック（Fallback） | 本来の方法が失敗したとき代替手段に切り替える | 主系 → 代替 | プロセス並列が使えない環境で逐次処理に切り替える |
> | フェールオーバー（Failover） | 障害時に同等の待機系へ自動切替する | 主系 → 待機系 | データベースのプライマリが落ちたらレプリカに切り替わる |
> | フェールバック（Failback） | フェールオーバー後に主系へ復帰する | 待機系 → 主系 | 復旧したプライマリに運用を戻す |
> | フォールトトレラント（Fault Tolerant） | 一部が壊れても全体が動き続ける設計思想 | （上位概念） | 冗長化・エラー回復・graceful degradationの組合せ |
>
> 研究用スクリプトでは、フェールオーバーやフェールバックが必要になる場面は少ない。一方、フォールバック（本節のCLIデフォルト値の例や、[§17](./17_performance.md)の並列処理フォールバック）とフォールトトレラントな設計（不正な入力行をスキップして処理を続行する等）は実務で頻繁に使う。エージェントがこれらの用語を使い分けているかをレビュー時に確認しよう。
...
> <a id="column-ch11-02"></a> **🧬 コラム: バイオインフォCLIツールの設計に学ぶ**
>
...
> <a id="column-ch11-03"></a> **🧬 コラム: パラメータ爆発問題**
>
...
> <a id="column-ch11-04"></a> **🤖 コラム: 機械学習CLIの設計パターン**
>
