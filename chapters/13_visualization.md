...
[§1-1 設計原則とプログラミングパラダイム](./01_design.md#設計原則とプログラミングパラダイム)で紹介した**オブジェクト指向**を、可視化APIへ応用したものがこの明示的APIである。MatplotlibのFigureとAxesは、図の状態とそれを操作するメソッドをまとめたオブジェクトであり、`ax.set_xlabel()`のように「対象.操作」の形で扱う。

ここでオブジェクト指向APIを使うことは、利用者が自分でクラスを設計することと同じではない。ライブラリが用意したオブジェクトを明示的に受け取り、どの図を操作しているかをコード上で明らかにする使い方である。
...
> <a id="column-ch13-01"></a> ### 🧬 コラム: バイオインフォの専門可視化ツール
>
...
> <a id="column-ch13-02"></a> ### 🧬 コラム: ゲノムトラックの作成とブラウザでの可視化
>
> #### ゲノムトラックとは
>
...
> <a id="column-ch13-03"></a> ### 🤖 コラム: 機械学習の可視化ツール
>
