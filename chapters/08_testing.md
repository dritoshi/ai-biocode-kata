...
#### ルールをディレクトリ単位で調整する — per-file-ignores

有効化したルールは、すべてのファイルに一律で適用される。しかしコードの種類によっては、一部のルールが過剰なこともある。たとえば docstring（`D`）は公開する関数やクラスには重要だが、`test_empty_sequence` のように名前で意図が明らかなテストメソッドにまで必須とすると、形式的な docstring を量産することになり、テストが数百に達するプロジェクトでは負担が大きい。

このような場合、`per-file-ignores` で特定のパスにだけルールを緩められる。

```toml
[tool.ruff.lint.per-file-ignores]
"tests/**" = ["D"]   # テストには docstring 必須ルールを適用しない
```

テストにも docstring を付けるのは依然としてよい習慣であり（後述の[docstringの書き方](#docstringの書き方)を参照）、テストが少ないうちは付けておくと、何を検証するテストかが一目で分かる。一方、テストが大量になったプロジェクトでは、上記のようにテストディレクトリを `D` の対象から外す運用が一般的である。リント・フォーマット用のスクリプトなど、書籍やライブラリ本体とは別の「裏方」コードについても、同様に一部ルールを緩めることがある。プロジェクトの規模と方針に応じて選べばよい。
...
> <a id="column-ch08-01"></a> **🧬 コラム: バイオインフォマティクスのテストデータ**
>
