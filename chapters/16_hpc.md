...
> 「大地よ海よ　そして生きているすべてのみんな……このオラにほんのちょっとずつだけ元気をわけてくれ……！！！」
> — 孫悟空, 鳥山明『ドラゴンボール』其之二百三十三, 第20巻 (集英社, 1990)

> **この章の前提知識**: SSHでリモートサーバに接続したことがあること。不安な場合は [§2 ターミナルとシェルの基本操作](./02_terminal.md) を復習し、所属機関のHPCガイドを参照するとよい。

[§15 コンテナによるソフトウェア環境の再現](./15_container.md)では、DockerやApptainerで解析環境を固定する方法を学んだ。環境の再現性が確保できても、数十サンプルのRNA-seqアライメントや数百パターンのハイパーパラメータ探索を手元のノートPCで実行するのは現実的ではない。こうした大規模計算を効率的に実行するためのインフラが、HPC（High-Performance Computing）クラスタである。
...
本章では、HPCクラスタの基本構成、Slurmジョブスケジューラの使い方、リモート接続とファイル転送、データ管理・バックアップの実践を学ぶ。また、HPCの先にあるクラウドコンピューティング（AWS、GCP等）の基本用語も概観する。
...
![HPCクラスタのアーキテクチャ: ローカルPC → ログインノード → Slurm → 計算ノード群、共有ファイルシステム](../figures/ch16_hpc_architecture.png)
...
利用可能なパーティションは `sinfo` コマンドで確認できる（詳細は[16-2節](#16-2-slurm)で学ぶ）。
...
以下のコマンドで、完了済みジョブの実際のリソース使用量を確認できる。`sacct`はSlurmに蓄積されたジョブの実行実績を表示するコマンド[1](https://slurm.schedmd.com/sacct.html)で、`--format`で表示する項目を指定する。
...
> **レビューの勘所: AIは専門用語を直訳しがち**
>
> AIコーディングエージェントに Slurm の説明を書かせると、`sacct` の "accounting" を「会計」と訳すことがある。一般に accounting は経理・簿記を指すが、Slurm では**ジョブの資源使用実績の記録**を意味する。エージェントは辞書的に最も一般的な訳を当てがちで、こうしたドメイン特有の意味を取り違えやすい。生成された文章は、専門用語が文脈に合っているかを人間がレビューして直す必要がある——本書が繰り返し説く判断の一例である。
...
`squeue`の出力に表示されるジョブの状態コードの主要なものを以下に示す。なお、`sacct` はアカウンティング機能（ジョブ実績の記録）が有効なクラスタで使えるコマンドであり、施設によっては `seff` や独自の監視ツールが案内されることもある。
...
デバッグや小規模なテストでは、計算ノード上でシェルを直接操作したいことがある。`srun --pty bash` を使うと、計算ノード上でインタラクティブなBashセッションを開始できる。`--pty`は疑似端末を割り当てるオプションである。施設によっては `salloc` で資源確保してからシェルを開く運用が標準の場合もあるため、ローカルルールを確認する。
...
GPU計算を行うには、`--gres=gpu:N` でGPUの数を申請する。`gres` は Generic Resource の略で、GPUに限らず特殊なリソースを申請するための仕組みである。近年のSlurmでは `--gpus` 系オプションも使えるが、どちらが有効かは施設設定に依存する。
...
> | 方式 | 仕組み | 例 |
> |------|--------|-----|
> | TOTP（時間ベースワンタイムパスワード） | スマートフォンアプリが30秒ごとに6桁のコードを生成 | Google Authenticator, Microsoft Authenticator |
> | パスキー（FIDO2 / WebAuthn） | 生体認証（指紋・顔）やハードウェアキーで認証 | Touch ID, YubiKey |
>
> HPCを利用する前に、所属機関の認証方式を確認し、必要なアプリやデバイスを準備しておくとよい。
...
モジュールシステムで提供されていないツールが必要な場合は、[§6 Python環境の構築](./06_dev_environment.md)で学んだconda環境を使う。ただし、HPCによってはホームディレクトリの容量制限でconda環境が圧迫されることがある。その場合は、ワーク領域やスクラッチにconda環境を配置する。
...
HPCの共有ファイルシステム（Lustre、GPFS等）は、大きなファイルの連続読み書きに最適化されている。逆に、大量の小ファイル（数十万個のファイル）の操作は性能が極端に低下し、他のユーザーにも影響する。[§3 コーディングに必要な計算機科学](./03_cs_basics.md)で学んだファイルシステムの仕組みを思い出してほしい。
...
- **ジョブ内ではローカルSSDを使う**: 具体的には `$TMPDIR` を使い、小ファイルの読み書きが多い処理はノードローカルで実行する
...
ディレクトリ名に日付・プロジェクト名を含め、見ただけで内容がわかるようにする。[§10 ソフトウェア成果物の設計](./10_deliverables.md)で学んだプロジェクト構成の考え方をHPC上でも適用する。
...
各プロジェクトディレクトリの直下にREADMEを置き、データの概要・取得元・処理状況を記録する。以下はテンプレートの抜粋である（完全版は `scripts/ch16/project_readme_template.md` を参照）。
...
- [§7 Git入門](./07_git.md)でスクリプトを管理し、組織のGitHubリポジトリに格納する
- 退職・異動後もデータにアクセスできる体制を整える

→ 投稿前のバックアップ・共有の最終確認は[付録C 論文投稿前チェックリスト](./appendix_c_checklist.md)を参照。
...
>
> 「クラウド」と聞くとDropboxやGoogle Driveのようなファイル共有サービスを思い浮かべるかもしれない。しかしバイオインフォマティクスの文脈でクラウドという場合、多くはAWS（Amazon Web Services）、GCP（Google Cloud Platform）、Microsoft Azureといった**インフラとしてのクラウド**を指す。これらは単なるファイル保管場所ではなく、計算機そのものを必要なときに必要なだけ借りられるサービスである。この違いを理解するには、クラウドサービスの3つの層を知っておく必要がある。
>
> | 層 | 正式名称 | 何を提供するか | 具体例 | ユーザーが管理するもの |
> |---|---------|-------------|-------|-------------------|
> | **SaaS** | Software as a Service | すぐ使えるアプリケーション | Dropbox、Google Drive、Gmail、wandb | 何もない（使うだけ） |
> | **PaaS** | Platform as a Service | コードを動かす環境。サーバの管理は不要 | AWS Batch、Google Cloud Run、Terra | コードとデータ |
> | **IaaS** | Infrastructure as a Service | 仮想マシンそのもの。OSから自分で構築する | AWS EC2、GCP Compute Engine | OS、ミドルウェア、コード、データのすべて |
>
> 普段使っているDropboxやGoogle DriveはSaaSであり、ファイルを置けば同期される。一方、バイオインフォマティクスで「クラウドで解析する」とは、多くの場合PaaS（AWS BatchやTerraにジョブを投入する）またはIaaS（EC2インスタンスを起動してSSHでログインし、HPCと同じように使う）を意味する。HPCクラスタが所属機関から無料で提供されるのに対し、IaaS/PaaSは使った分だけ課金される点が大きな違いである。
>
> その他の重要な用語:
>
> | 用語 | 意味 | HPCとの対比 |
> |------|------|-----------|
> | **オブジェクトストレージ**（S3, GCS） | ファイルをHTTP経由でバケット（入れ物）に保存する仕組み。`s3://bucket-name/path/to/file.fastq.gz`のようなURIでアクセスする | HPCのファイルシステム（NFS, Lustre）に相当 |
> | **リージョン** | データセンターの物理的な所在地（例: `ap-northeast-1`は東京、`us-east-1`はバージニア）。データとコンピュートを同じリージョンに置かないと転送コストが発生する | HPCはすべて同一施設内 |
> | **インスタンス** | 仮想マシン。CPU・メモリ・GPUの組み合わせでタイプを選ぶ（例: `m5.xlarge`は4 vCPU, 16 GB RAM）。使った時間だけ課金される | HPCのジョブに相当するが、起動・停止を自分で管理 |
>
> NCBIはシーケンスデータをAWS S3とGoogle Cloud Storageで公開しており、`aws s3 cp s3://sra-pub-run-odp/sra/SRR000001/ . --no-sign-request`のようにダウンロードできる。[§19 公共データベースとAPI](./19_database_api.md)で学ぶEBI/NCBIのデータ取得と合わせて覚えておくとよい。
>
> クラウドの詳細な使い方（アカウント作成、IAM設定、VPC構築等）は本書のスコープ外であるが、各クラウドプロバイダの無料枠（AWS Free Tier、GCP Free Tier）を使えば、小規模なデータで試すことは可能である。なお、各クラウドにはCLIツール（`aws`コマンド、`gcloud`コマンド、`az`コマンド）が用意されており、本書で学んだAIコーディングエージェントからこれらのCLIを呼び出してクラウドリソースを操作することもできる。例えば「S3にFASTQをアップロードして」「EC2インスタンスを起動して」といった指示をエージェントに出す場面があり得るだろう。
...
次章の[§17 コードのパフォーマンス改善](./17_performance.md)では、プログラムの実行時間とメモリ使用量を計測・改善する方法——プロファイリング、アルゴリズムの選択、並列化——を学ぶ。
...
### 演習 16-1: Slurm リソース申請 **[概念]**

以下のジョブスクリプトの問題点を指摘せよ。
...
python simple_filter.py input.vcf > output.vcf
```

具体的に、リソース申請の各項目（CPU、メモリ、実行時間）が実際のジョブに対して適切かを評価せよ。

（ヒント）シングルスレッドの Python スクリプトに64コアと500GBメモリは過剰である。`sacct` で過去のジョブ実績を確認し、実際の使用量に基づいてリソースを調整する習慣が重要である。

### 演習 16-2: アレイジョブの指示 **[指示設計]**

20サンプルの BWA マッピングをアレイジョブとして実行するスクリプトを生成する指示文を書け。以下の要件を含めること。

- CPU 4コア、メモリ 16GB を各ジョブに割り当てる
- `--array=1-20` でアレイジョブを設定する
- `SLURM_ARRAY_TASK_ID` からサンプル名を取得する
- ログファイルにアレイタスク ID を含める

（ヒント）サンプル名のリストファイル（`samples.txt`）を用意し、`sed -n "${SLURM_ARRAY_TASK_ID}p" samples.txt` でタスク ID に対応するサンプル名を取得するパターンが一般的である。

### 演習 16-3: データ転送戦略 **[設計判断]**

50GB の FASTQ ファイル群をローカルマシンから HPC クラスタに転送する。以下の3つの方法から最適なものを選び、理由を述べよ。

- (a) `scp` — セキュアコピー
- (b) `rsync` — 差分転送
- (c) Globus — グリッドファイル転送サービス

（ヒント）`rsync` は差分転送と中断再開（`--partial`）が可能で、大規模ファイル転送に最適である。転送が途中で失敗しても、再実行すれば残りのファイルだけを転送できる。

---

## さらに学びたい読者へ

本章で扱ったHPCクラスタでの計算実行をさらに深く学びたい読者に向けて、公式ドキュメントと教科書を紹介する。

### ジョブスケジューラ

- **SchedMD. "Slurm Workload Manager Documentation".** https://slurm.schedmd.com/documentation.html — Slurmの公式ドキュメント。sbatch/srun/sacctの全オプション、配列ジョブ、ジョブ依存関係指定の完全なリファレンス。
