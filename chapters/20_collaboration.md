...
本章では、データとコードを安全に管理するための知識——シークレット管理、エージェントハーネス、ヒトデータの法規制と匿名化——を学んだ。本章では、その知識を土台にして、他の研究者やコミュニティとの**協働**の技術を学ぶ。

コミュニティとの対話スキル——質問の構成、Issue報告、レビューコメント、進捗報告——を身につければ、エージェントに下書きを任せ、科学的な判断とコミュニケーションの精度に集中できる。これらのスキルがあれば、質問は回答してもらいやすく、Issue報告は適切になり、共同研究者との認識齟齬が少なくなりコミュニケーションコストを下げられる。エージェントは環境情報の収集・質問文の構成・PRの説明文の下書き・進捗報告の雛形作成を得意とする。一方、「何が問題の本質か」の切り分け、レビューコメントの科学的妥当性の判断、共同研究者に何を伝えるべきかの優先順位づけは人間が行う。

本章では、まず[20-1節](#20-1-質問の技術--正しく助けを求める)で質問の構成と質問先の使い分けを学ぶ。次に[20-2節](#20-2-コードレビューとossコミュニティへの参加)でコードレビューの作法とOSSコミュニティへの参加方法を扱う。最後に[20-3節](#20-3-共同研究のコミュニケーション--piと共同研究者への共有)で共同研究者やPIへの進捗共有と解析依頼の受け取り方を解説する。

---

## 20-1. 質問の技術 — 正しく助けを求める
...
**最小再現例**（Minimal Reproducible Example; MRE）とは、問題を再現するために必要な最小限のコードである。[§16 デバッグ](./16_debug.md)で学んだ「問題の局所化」技法の応用であり、自分のプロジェクト固有のコードを削ぎ落として、問題の本質だけを残す作業である。
...
```python
# scripts/ch20/format_question.py — 環境情報の自動収集と質問テンプレート生成（抜粋）
...
完全なコードは`scripts/ch20/format_question.py`にある。

#### エージェントへの指示例

エラーに遭遇した際、エージェントに質問文の下書きを任せれば、必要な情報の収集漏れを防げる。ただし、問題の本質が正しく伝わっているかの確認は人間が行う。

> 「以下のエラートレースバックから、Stack Overflowに投稿する質問文を作成して。環境情報、最小再現例、期待する結果と実際の結果を含めて。英語で書いて」

> 「このスクリプトが特定のFASTQファイルでだけ失敗する。問題を再現するための最小限のテストデータ（5リード程度）を生成して」

> 「`scripts/ch20/format_question.py`を使って、biopython、pandas、numpyのバージョン情報を含む質問テンプレートを生成して」
...
[§6-2 GitHubの活用](./06_git.md#6-2-githubの活用)で学んだIssueの作成は、特定のツールやライブラリのバグ報告に適している。バグ報告Issueの構造は20-1-1節の質問構成と同じだが、加えて以下を含める:
...
質問先によって求められるフォーマットや語調が異なる。エージェントに質問先を指定して下書きさせると、フォーマットへの準拠を効率化できる。

> 「この問題をBiostarsに投稿したい。使用ツール（STAR 2.7.11a）、データの種類（paired-end RNA-seq）、リファレンスゲノム（GRCh38）の情報を含めた質問文を英語で作成して」

> 「このバグをGitHub Issueとして報告する文章を英語で書いて。環境情報、再現手順、期待される動作、実際の動作を含めて。最新版（v2.1.0）で再現することも明記して」
...
**XY問題**（XY Problem）とは、本当に解決したい問題Xではなく、自分が思いついた解決策Yについて質問してしまう現象である[6](https://doi.org/10.1371/journal.pcbi.1005510)。
...
> **エチケット:**
>
> - **マルチポスティング禁止**: 同じ質問をBiostarsとStack Overflowの両方に投稿しない。どちらかに絞る
...
> **AI生成回答に関する倫理:**
>
> Stack Overflowは2022年12月に、AI（ChatGPT等）で生成した回答の投稿を制限するポリシーを導入した[7](https://meta.stackoverflow.com/questions/421831)。理由は、AI生成回答は一見もっともらしいが、技術的に不正確な内容を含む場合があり、レビューの負担が増大するためである。Biostarsでも同様の問題意識が共有されている。
>
> 自分の質問を整理するためにエージェントを使うことは問題ないが、エージェントの出力を検証せずにそのまま回答として投稿することは、コミュニティの信頼を損なう行為である。

---

## 20-2. コードレビューとOSSコミュニティへの参加

### 20-2-1. コードレビューのコミュニケーション

[§0-4 レビューの技法](./00_ai_agent.md)ではエージェントにコードレビューを依頼する手法を、[§6-2 GitHubの活用](./06_git.md#6-2-githubの活用)ではPull Requestの作成手順を学んだ。本節では、**人間同士の**コードレビューにおけるコミュニケーションの作法を扱う。GitHub Docs[3](https://docs.github.com/en/pull-requests/collaborating-with-pull-requests/reviewing-changes-in-pull-requests/about-pull-request-reviews)のPull Requestレビューガイドも参考になる。
...
```python
# scripts/ch20/review_helper.py — PR説明文生成ヘルパー（抜粋）
...
完全なコードは`scripts/ch20/review_helper.py`にある。
...
バグ報告は、20-1-1節で学んだ質問技術の応用である。質問との違いは、「自分の使い方が間違っている可能性」と「ソフトウェアにバグがある可能性」の両方を公平に扱う点にある。
...
[§5 開発環境](./05_dev_environment.md)で学んだcondaのバイオインフォチャンネルである**bioconda**[4](https://bioconda.github.io/contributor/index.html)は、コミュニティドリブンで運営されている。新しいツールのレシピ（パッケージ定義ファイル）を追加するPRや、既存ツールのバージョン更新PRは、初心者でも取り組みやすい貢献形態である。
...
biocondaレシピの作成は、テンプレートが定まっているためエージェントに任せやすい。ただし、ハッシュ値の計算や依存関係の正確性は人間が確認する。
...
OSSに貢献する際、あるいは他者のOSSを自分のプロジェクトに取り込む際には、ライセンスの理解が不可欠である。ライセンスの選択基準については[§6-2 GitHubの活用](./06_git.md#6-2-githubの活用)を参照されたい。
...
| Apache-2.0 | MIT | ✅ 互換（Apacheの帰属表示条件を守る） |
| GPL-3.0 | GPL-3.0 | ✅ 互換 |

基本的な原則は「コピーレフト（GPL等）のコードを取り込むと、自分のプロジェクトもコピーレフトにする必要がある」ということである。パーミッシブライセンス（MIT、BSD、Apache-2.0）同士であれば、互換性の問題はほとんど生じない。
...
## 20-3. 共同研究のコミュニケーション — PIと共同研究者への共有
...
[§17 ドキュメント化](./17_documentation.md)で学んだCHANGELOG管理の応用として、git logから「先週やったこと」を自動的に要約するアプローチがある。
...
```python
# scripts/ch20/progress_report.py — 進捗報告の自動生成（抜粋）
...
完全なコードは`scripts/ch20/progress_report.py`にある。
...
> 「`scripts/ch20/progress_report.py`を使って、git logから今週の報告書を生成して」
...
これらの記録には、[§17 ドキュメント化](./17_documentation.md)で学んだANALYSIS_LOG.mdが役立つ。

#### 生物学的解釈の添え方

解析結果を報告する際は、数値だけでなく**生物学的文脈**を添える。[§11 可視化](./11_visualization.md)で学んだ図表作成技術を活用して、視覚的に分かりやすく伝える。
...
```python
# scripts/ch20/analysis_intake.py — 解析依頼チェックリスト（抜粋）
...
[§19-2 データの倫理](./19_security_ethics.md#19-2-データの倫理)のデータ管理計画（DMP）で定めた共有ルールに従い、ヒトデータの場合は倫理審査の承認やDUAの確認も解析前チェックリストに含める。

完全なコードは`scripts/ch20/analysis_intake.py`にある。
...
> - カラーバーや凡例を必ず付ける。色覚多様性に配慮したカラーパレットを使う（[§11 可視化](./11_visualization.md)参照）
>
...
[7] Stack Overflow. "Temporary policy: Generative AI (e.g., ChatGPT) is banned". https://meta.stackoverflow.com/questions/421831 (参照日: 2026-03-22)
