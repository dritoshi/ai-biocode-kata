...
[§1 設計原則](./01_design.md#1-2-unix哲学)で学んだUNIX哲学——テキストストリーム、パイプ、終了コード——はCLIツール設計の理論的基盤である。本章では、その概念をPythonで実装する具体的な技法を学ぶ。ライブラリ選定から始めて、サブコマンド設計、プログレス表示、ロギングまで、使いやすいCLIを作るための実践的なテクニックを一通り扱う。
