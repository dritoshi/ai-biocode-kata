...
![VM vs コンテナのアーキテクチャ比較: VMはゲストOSを含むが、コンテナはホストOSのカーネルを共有](../figures/ch15_vm_vs_container.png)
...
この「カーネル共有」にはLinuxの**名前空間**（namespace）と **cgroups**（control groups）が使われている。名前空間はプロセス・ネットワーク・ファイルシステムを分離し、cgroups はCPU・メモリ・I/O などの資源使用量を制御する。これにより、コンテナ内からはあたかも独立したLinux環境のように見える。
...
![Dockerfile → Image → Container: ビルドで不変のイメージを作成し、実行でコンテナを起動](../figures/ch15_dockerfile_flow.png)
...
# Miniforge3ベースでbiocondaパッケージを利用する
...
バイオインフォマティクスのDockerfileでは、**Miniforge3**（例: `condaforge/miniforge3`）系のベースイメージを使うのが実務的である。Mambaforge は非推奨化されたため、新規の例では Miniforge3 に揃える。理由は以下のとおり:
...
![Dockerレイヤーキャッシュの最適化: 変更頻度の低い操作を先に、高い操作を後に配置する](../figures/ch15_layer_cache.png)
...
1. Plan:   Dockerfileの構成を計画（読み取り専用モード）
           「Miniforge3ベースでSTAR, samtools, featureCountsを
...
> 「RNA-seq解析用のDockerfileを作成してください。ベースイメージはMiniforge3で、environment.ymlからconda環境を構築する構成にしてください。レイヤーキャッシュを意識した命令順序にしてください」
...
apptainer pull rnaseq.sif docker://condaforge/miniforge3:24.3.0-0
...
![コスト vs 再現性: README → ロックファイル → Docker → ダイジェスト+ワークフローの4段階](../figures/ch15_cost_vs_reproducibility.png)
...
[§14 解析パイプラインの自動化](./14_workflow.md#column-ch14-03)の🤖コラムでSnakemake/Nextflowとの使い分けを紹介した。ここでは、実験追跡に特化したツールの特徴を整理する:
...
[§14 解析パイプラインの自動化](./14_workflow.md#column-ch14-03)の🤖コラムで述べたとおり、配列解析パイプライン（外部ツール呼び出しが中心）は[Snakemake/Nextflow](./14_workflow.md#14-2-ワークフロー言語)、機械学習実験（Pythonコードが中心）はwandb/MLflowと使い分けるのが現実的である。両方を含むプロジェクトでは、Snakemakeのワークフロー内でMLflowやwandbのトラッキングを呼び出す組み合わせも有効である。
...
### 演習 15-1: Dockerfile のレビュー **[レビュー]**

エージェントが以下の Dockerfile を生成した。再現性の観点から問題点を指摘せよ。
...
具体的に、以下の3点について問題を説明し、改善案を示せ。

1. ベースイメージのタグ指定
2. `pip install` のバージョン指定
3. イメージサイズの最適化

（ヒント）`python:3.10-slim` のようにバージョンを固定し、さらに論文投稿レベルではダイジェスト（`@sha256:...`）で固定する。パッケージも `numpy==1.24.0` のようにバージョンを明示する。

### 演習 15-2: 再現性レベルの選択 **[設計判断]**

以下のプロジェクト段階ごとに、適切な再現性レベルを判断せよ。各段階で「どこまでの環境固定が必要か」を具体的に述べよ。

- (a) 個人の探索的解析
- (b) ラボ内での共有
- (c) 論文投稿
- (d) 異なる OS 環境（macOS / Linux）での実行

（ヒント）過剰な再現性パッケージングは YAGNI（You Ain't Gonna Need It）違反でもある。探索段階では conda/venv で十分な場合が多く、論文投稿時にコンテナ化すればよい。

### 演習 15-3: Docker と Apptainer の使い分け **[概念]**

ローカル開発環境では Docker を、HPC クラスタでは Apptainer を使うのが一般的である。この使い分けの理由を、セキュリティモデルの違いに基づいて説明せよ。

（ヒント）HPC では一般ユーザーが root 権限を持てない。Docker デーモンは root 権限で動作するため、共有環境では使用できない場合が多い。

### 演習 15-4: コンテナの動作確認 **[実践]**

エージェントに Biopython を含む Dockerfile を書かせ、以下の手順で動作確認せよ。

1. `docker build -t biotest .` でイメージをビルドする
2. `docker run --rm biotest python -c "import Bio; print(Bio.__version__)"` で動作を検証する
3. ビルドエラーが発生した場合、エラーメッセージをエージェントに渡してデバッグさせる

（ヒント）ビルドエラーの全文をエージェントにそのまま渡すことが重要である。エラーメッセージを要約せず、全文を共有すれば、エージェントは適切な修正を提案できる。
...
[3] da Veiga Leprevost, F. et al. "BioContainers: an open-source and community-driven framework for software standardization." *Bioinformatics*, 33(16), 2580–2582, 2017. [https://pubmed.ncbi.nlm.nih.gov/28379341/](https://pubmed.ncbi.nlm.nih.gov/28379341/)
