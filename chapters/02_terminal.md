...
`#!/usr/bin/env python3` は「PATHの中から `python3` を探して実行せよ」という意味である。`#!/usr/bin/python3` と直接パスを書く方法もあるが、`env` 経由のほうが環境の違いに対応しやすい（[§5 開発環境の構築](./05_dev_environment.md)で詳しく扱う）。
...
ジョブ管理（`&`、`nohup`、`bg`、`fg`）やスケジューリングの詳細は、HPC環境で必要になったときに[§14 HPC](./14_hpc.md)で扱う。
...
APIキーやデータベースパスなど、プロジェクト固有の設定は `.env` ファイルで管理する。**`.env` ファイルはGitにコミットしてはいけない**（[§19 セキュリティと倫理](./19_security_ethics.md)で詳述する）:
...
ただし、**本格的なCLIツールはPythonで書くべきである**（[§9 CLIツールの設計](./09_cli.md)参照）。シェルスクリプトは「接着剤」——既存のツールを順番に呼び出す程度に留めるのがよい。
...
**目安: 50行を超えるシェルスクリプトはPythonで書き直すことを検討する。** [§9 CLIツールの設計](./09_cli.md)でargparseやtyperを使った本格的なCLI開発を学ぶ。
...
>
> バイオインフォマティクスの解析は、ローカルPCではなくリモートサーバやHPCクラスタ上で行うことが多い。ここでは、リモート作業に最低限必要な3つのツールを紹介する。詳細な設定やジョブ管理は[§14 HPC](./14_hpc.md)で扱う。
>
> ### SSH — リモートサーバへの接続
>
