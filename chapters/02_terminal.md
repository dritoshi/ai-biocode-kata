...
> （自分で作れないものは、理解していないということだ。）
> — Richard Feynman, カリフォルニア工科大学の黒板に残された言葉 (1988)
...
> <a id="column-ch02-01"></a> **📦 コラム: エージェントの内部で動く高速検索ツール — ripgrepとfd**
>
...
> <a id="column-ch02-02"></a> **📦 コラム: SSH・tmux・Vim — サーバ上でのVibe coding**
>
...
> ```
>
> 長時間かかるバイオインフォの解析（マッピング、アセンブリなど）は、必ずtmux内で実行する。リモートサーバ上でAIエージェント（Claude Code、Codex等）を使う場合も同様である。エージェントとの対話セッションをtmux内で実行すれば、SSH接続が切れてもセッションは維持される。両ツールともresume機能（中断したセッションの再開）を備えているが、tmuxならセッション自体が途切れないためより確実である。
>
> ### Vim — サーバ上でのファイル編集
>
...
エージェントが以下のコマンドを実行した。各部分が何をしているか説明せよ。

```bash
find data/ -name '*.fastq.gz' -size +1G | wc -l
```

（ヒント）パイプ `|` の前後でそれぞれ何が起きているかを分解して考える。

### 演習 2-2: パーミッションの問題 **[レビュー]**

エージェントが以下のコマンドを提案した。

```bash
chmod 777 analysis.sh
```

このコマンドの問題点を述べ、より適切なパーミッションを提案せよ。

（ヒント）777は全ユーザーに読み・書き・実行の全権限を付与する。共有サーバーでの影響を考える。

### 演習 2-3: ファイル整理の指示 **[指示設計]**

以下の状況を想定する。カレントディレクトリに複数サンプルのFASTQファイルがフラットに置かれている:
...
これらをサンプルごとにサブディレクトリに整理するためのエージェントへの指示文を作成せよ。ファイル命名規則やディレクトリ構成の要件を含めること。

（ヒント）ファイル名のパターン（例: `sampleA_R1.fastq.gz`）を指示に含めると、エージェントが正しいグロブパターンを使える。

### 演習 2-4: 環境変数の確認 **[実践]**

エージェントに `echo $PATH` の結果を表示させ、以下を確認せよ:

1. conda環境やpyenvのパスが含まれているか
2. パスの優先順位が期待通りか
3. 意図しないパスが混入していないか
