...
> 「理解・分解・再構築。それが錬金術の基本だ。」
> — エドワード・エルリック, 荒川弘『鋼の錬金術師』第1巻 (スクウェア・エニックス, 2002)
...
本章では、バイオインフォマティクスでよく使われる9つの成果物パターンを整理し、それぞれの特徴と選び方を示す。Webアプリやドキュメントサイトの公開先（GitHub Pages、Streamlit Community Cloud等）についても扱う。続いて、pipパッケージの標準的なディレクトリ構成、設定管理、エラーハンドリングの作法を学ぶ。最後に、小さなスクリプトから始めて段階的にプロジェクトを成長させる方法を紹介する。
...
> <a id="column-ch10-01"></a> **🧬 コラム: Biocondaへのパッケージ公開**
>
...
ここで `tests/conftest.py` に出てくる**フィクスチャ**とは、pytest がテストの前に自動で準備してくれる共通データや共通オブジェクトのことである。複数のテストで同じ入力データ、一時ディレクトリ、モックオブジェクトを使い回すときに有効で、詳しい書き方は[§8-2 テストの実践](./08_testing.md#8-2-テストの実践)で扱う。
...
| `tests/conftest.py` | pytestの共通フィクスチャ。複数テストで使う準備コードを再利用する仕組み（[§8](./08_testing.md)参照） |
...
> <a id="column-ch10-02"></a> **🤖 コラム: 機械学習プロジェクトのディレクトリ構成**
>
...
バイオインフォマティクスのツールには、しばしば多くのパラメータがある。リード品質の閾値、マッピングのミスマッチ許容数、フィルタリング条件——これらをコード内にハードコーディングするのは、[§1-1 コードの設計原則](./01_design.md#実行条件への応用--設定を外から与える)で学んだ関心の分離の観点から避けるべきである。
