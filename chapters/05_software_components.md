...
┌──────────────┐    MCP     ┌──────────────────┐
│  AIエージェント │ ◄────────► │  MCPサーバー        │
│  (Claude Code  │  標準化された │  (GitHub, Notion,  │
│   / Codex)     │  プロトコル   │   ファイルシステム等) │
└──────────────┘            └──────────────────┘
```
...
- **Bryant, R. E., O'Hallaron, D. R. *Computer Systems: A Programmer's Perspective* (3rd ed.). Pearson, 2015.** — CMU（カーネギーメロン大学）の教科書（通称 CSAPP）。プログラムのコンパイル・リンク・ロード・実行の仕組みをプログラマの視点から解説する。特に第7章 "Linking" が本章と直結する。邦訳: 鹿野桂一郎訳『コンピュータシステム 基礎から応用まで プログラマの視点で学ぶ（第3版）』東京化学同人, 2025.

### Pythonのモジュールとパッケージ

- **Beazley, D., Jones, B. K. *Python Cookbook* (3rd ed.). O'Reilly, 2013.** — Pythonのモジュール・パッケージ・importシステムに関する実践的レシピ集。特に Chapter 10（Modules and Packages）が本章と直結する。邦訳: 黒川利明訳『Python Cookbook 第3版』オライリー・ジャパン, 2014.
