...
本章では、プログラムの実行の仕組み（コンパイルとリンク）、Pythonのモジュールシステム、依存関係の問題、そしてAPIの概念を順に学ぶ。これらは[§6 開発環境の構築](./06_dev_environment.md)で `pip install` や `conda install` を使いこなすための前提知識となる。
...
🧬 **condaと共有ライブラリ**: condaは、Pythonパッケージだけでなく、C/C++製ツールとその依存共有ライブラリもconda環境内にインストールする。biocondaチャネルからsamtoolsをインストールすると、libhtsやlibzもconda環境の `lib/` ディレクトリに配置される。これにより、システムのライブラリバージョンに依存しない。ただし、conda環境外のツールとの相互作用や、OSレベルのライブラリ（CUDA等）までは管理できない。完全な環境の隔離が必要な場合は、[§15 コンテナと再現性](./15_container.md)で学ぶコンテナを使う。
...
仮想環境をアクティベートすると、その環境の site-packages が `sys.path` の先頭付近に追加される。これにより、プロジェクトごとに異なるバージョンのパッケージを使い分けられる。詳しくは[§6 開発環境の構築](./06_dev_environment.md#6-1-pythonの環境管理)を参照。
...
推奨されるのは、[§10 成果物の形式とプロジェクト設計](./10_deliverables.md)で学ぶ `pip install -e .`（開発モードインストール）である。`pyproject.toml` にパッケージ情報を記述し、`pip install -e .` でsite-packagesにリンクを作成すれば、PYTHONPATHを操作せずに自作モジュールをインポートできる。
...
2. **環境の分離**: プロジェクトごとに仮想環境を分ける（[§6 開発環境の構築](./06_dev_environment.md)）
3. **コンテナの利用**: OSレベルの依存まで含めて隔離する（[§15 コンテナと再現性](./15_container.md)）

### 環境隔離で解決できること・できないこと

[§6 開発環境の構築](./06_dev_environment.md)で学ぶvenvやcondaと、[§15 コンテナと再現性](./15_container.md)で学ぶコンテナでは、隔離できる範囲が異なる:
...
コマンド名、サブコマンド、オプション、引数の組み合わせが、CLIツールの「API」である。詳しくは[§11 CLIツールの設計](./11_cli.md)を参照。
...
URLの構造、HTTPメソッド、パラメータ、レスポンス形式が、Web APIの「API」である。詳しくは[§19 データベースとAPI](./19_database_api.md)で扱う。

### APIの変更がコードを壊す

エージェントが生成するコードは、ライブラリのAPIを使っている。ライブラリのバージョンが上がってAPIが変わると、以前は動いていたコードが壊れる。これが[§7 バージョン管理](./07_git.md#7-4-セマンティックバージョニング)で学ぶセマンティックバージョニングが重要な理由の一つである:
...
| 概念 | 意味 | 本書での関連箇所 |
|------|------|-----------------|
| コンパイル・リンク | ソースコードを実行ファイルに変換する過程 | [§15 コンテナと再現性](./15_container.md)（共有ライブラリの問題） |
| 共有ライブラリ | 複数プログラムで共用される動的ライブラリ | [§6 開発環境の構築](./06_dev_environment.md)（conda管理） |
| モジュール | 1つの `.py` ファイル | [§10 成果物の形式とプロジェクト設計](./10_deliverables.md)（プロジェクト構造） |
| パッケージ | `__init__.py` を含むディレクトリ | [§10 成果物の形式とプロジェクト設計](./10_deliverables.md)（pip パッケージ） |
| `sys.path` | Pythonのモジュール検索パス | [§6 開発環境の構築](./06_dev_environment.md)（仮想環境） |
| 依存関係地獄 | バージョン要求の衝突 | [§6 開発環境の構築](./06_dev_environment.md)（環境隔離） |
| API | ソフトウェアの利用窓口 | [§11 CLIツールの設計](./11_cli.md)、[§19 データベースとAPI](./19_database_api.md) |
...
次に進む章として、環境管理の実践は[§6 開発環境の構築](./06_dev_environment.md)、プロジェクト構造の設計は[§10 成果物の形式とプロジェクト設計](./10_deliverables.md)、コンテナによる完全な隔離は[§15 コンテナと再現性](./15_container.md)を参照してほしい。
