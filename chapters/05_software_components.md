...
ここで重要なのがリンクの段階である。ほとんどのプログラムは、すべての機能を自前で実装しているわけではない。圧縮処理にはzlib、数値計算にはBLASといった外部ライブラリの機能を「借りて」いる。リンクとは、この「借りる」関係を解決する処理である[4](https://shop.elsevier.com/books/linkers-and-loaders/levine/978-0-08-051031-6)。
