...
| 安全制御 | なし（人間が手動で適用） | 承認ポリシーとサンドボックスを組み合わせて制御 |
...
もちろん、ブラウザチャットが適している場面もある。概念の説明を聞きたいとき、エラーメッセージの意味を解読したいとき、コードの断片について質問したいとき——こうした「相談」にはブラウザチャットの手軽さが向いている。くわえて、計画フェーズで最新のベストプラクティスやツール比較を多段階で調べたい場面では、ブラウザ側が提供する Deep Research 系のエージェント的調査機能を併用するのがよい（詳しくは[§0-2 計画フェーズの外部調査 — Deep Researchの活用](#計画フェーズの外部調査--deep-researchの活用)で扱う）。CLIエージェントはブラウザチャットを否定するものではなく、「手を動かす」作業を任せるための道具である。本書では、この「手を動かせる共同作業者」としてのCLIエージェントを前提に、ソフトウェア開発の作法を学んでいく。
...
AIコーディングエージェントの利用を始めるには、ツールのインストールと認証が必要である。配布形態はツールごとに異なるため、ここでは macOS / Linux 環境向けの代表的な導入方法だけを示す。まずターミナルを起動して以下のコマンドを実行する。
...
Claude Code は操作モードを前面に出すのに対し、Codex CLI は `--sandbox` と `--ask-for-approval` の2軸で制御する。したがって、両者を1対1の「3つの承認モード」とみなすのは正確ではない。以下では、読者が実際に使い分けやすい対応関係で整理する。
...
### 計画フェーズの外部調査 — Deep Researchの活用

計画フェーズで行う調査には大きく2種類がある。ひとつはプロジェクト内のコードベースを読み解く**内部探索**であり、これは読み取り専用モードのエージェントが最も得意とする領域である。もうひとつは、最新のベストプラクティスやツール比較、論文の動向といった**外部知識の取り込み**であり、こちらは CLI セッションの中で完結させるより、別の道具に任せたほうが効率がよいことが多い。

外部情報を取り込む道具は、大まかに3つのレイヤに分けて考えられる。

| レイヤ | 反復回数 | 所要時間 | 想定用途 |
|--------|---------|----------|----------|
| モデル内部の知識 | 0〜1 | 数秒 | 用語の意味、関数仕様の即答、一般論の確認 |
| インラインWeb検索 | 1 | 数秒〜十数秒 | 公式ドキュメント・API仕様の最新版確認、単発の事実確認 |
| **Deep Research** | 自動で多段反復 | 数分〜1時間 | ベストプラクティス比較、ツール比較、論文サーベイ、ドメイン全体の地図化 |

CLI エージェントが内蔵するWeb検索ツールは上記の2番目のレイヤに相当し、「実装の手を止めずに1つの事実を確認する」ような用途に向いている。一方、バイオインフォマティクスでよくある「この解析に最適なツールはどれか」「2025年以降に提案された手法を俯瞰したい」といった問いは、複数の検索クエリを自動的に立てて結果に応じて方針を更新する仕組み——**Deep Research**（以下 DR）——のほうが向いている。本節ではこの3レイヤ目を計画フェーズにどう組み込むかを説明する。
...
#### Vibe codingワークフローにおける位置付け

DR は計画フェーズの**最上流**に置くのが自然である。すなわち、インタビューパターン（次項）で自分の頭の中にある要件を引き出す**前に**、まず DR で外部知識の地図を描いておき、その結果を仕様書の下敷きにするという流れである。具体的なワークフローは次のようになる。

```
1. ブラウザチャット側でDRに調査を依頼（10〜60分、別ウィンドウで放置）
  ↓
2. 生成されたレポートを人間がレビューし、引用先を一次情報まで確認
  ↓
3. 信頼できる情報だけを仕様書（docs/spec.md）に転記。実行日時と
   使用したDRサービス名、プロンプトを併記
  ↓
4. 新しいCLIセッションを開き、仕様書を渡してインタビューパターンや
   実装に進む
```

この流れのポイントは、DR の長時間の調査結果が CLI セッションのコンテキストウィンドウを圧迫しないことである。[§0-6 コンテキスト管理](#0-6-コンテキスト管理)で扱うように、CLI エージェントのコンテキストは貴重なリソースであり、レポート1本で数万トークンを消費するような調査は、最初からセッションの外で行うほうが経済的である。また、レポートを人間が一度フィルタに通すことで、DR が混入させうる誤情報が実装に直接流れ込むのを防げる。

#### バイオインフォマティクスでの典型的な使いどころ

DR が最も威力を発揮するのは、**計画フェーズで複数のツールを比較し、どれを採用するか判断するため**の調査である。バイオインフォマティクスは新しい手法が次々と提案される分野であり、「数年前に聞いたツールが今も現役か」「似たツールが乱立しているがベンチマーク上どれが強いか」といった問いに答えるには、単発のWeb検索では不十分なことが多い。典型的な問いを3つ挙げる。

- **ChIP-seqのピークコーラを比較したい**: MACS3、SEACR、GoPeaks、LanceOtron 等の候補について、想定シグナル形状（ナロー/ブロード）、必要な入力コントロール、解像度、計算コスト、最新ベンチマークでの性能を比較する
- **ヒト体細胞 SNV/Indel 検出のバリアントコーラを選びたい**: Mutect2、Strelka2、VarScan2、DeepVariant、Octopus の感度・特異度・計算コスト・WGS/WES 適合性を、複数のベンチマーク論文を横断して整理する
- **scRNA-seq の ambient RNA 除去ツールを選びたい**: SoupX、CellBender、DecontX、scAR を、著者らの主張ではなく独立評価論文の結果を中心に比較する

いずれも「手を動かす前に道具を選ぶ」ための調査であり、結果は仕様書の「採用ツールとその根拠」セクションにそのまま反映できる。
...
DR に投げるプロンプトは、CLI エージェントへの指示とはやや異なる作法が要る。短時間で終わるインラインWeb検索と違い、DR は調査範囲を広くとりすぎるとレポートが散漫になり、逆に狭すぎると知りたい比較軸が抜け落ちる。以下の3点を満たすプロンプトが扱いやすい。

- 目的・対象生物種・データ型・既知の制約・出力形式を最初に明示する
- 比較軸を先に列挙する（「以下4軸で比較せよ: 精度、スループット、依存関係、ライセンス」）
- 引用必須を明示する（「引用の無い主張は推測と明示せよ」）

> 「ヒトの体細胞SNV/Indel検出に使われる主要バリアントコーラ（Mutect2、Strelka2、VarScan2、DeepVariant、Octopus）を比較して。比較軸は感度、特異度、計算コスト、WGS/WES適合性の4つ。独立した評価論文の結果を中心に、各ツールに公式リポジトリと代表論文のDOIを必ず付けて。最後にWGS用・WES用それぞれの推奨ツールを根拠付きで提示して」

> 「single-cell RNA-seqのambient RNA除去ツール（SoupX、CellBender、DecontX、scAR）を比較して。著者らの主張ではなく独立評価論文の結果を中心に。出力は比較表＋200字の要約＋論文DOIリスト。引用の無い主張は『推測』と明示して」

> 「ChIP-seqのピークコーリングについて、2024年以降に更新された主要ツール（MACS3、SEACR、GoPeaks、LanceOtron 等）を比較して。比較項目は想定シグナル形状、必要な入力コントロール、解像度、計算コスト、最新ベンチマークでの性能。各項目に一次情報の引用を必ず付けて。最後にナロー用・ブロード用それぞれの推奨ツールを根拠付きで提示して」

ただし、DR の出力を鵜呑みにしてはいけない。DR でもハルシネーションは完全には消えず、架空のDOIや誤った著者帰属、存在しない論文タイトルが混入することがある。レポート中の重要な主張は、引用先を必ず人間が開いて一次情報まで遡ること。また、DR はサブスクリプション契約でも実行回数に制限があり、1リクエストで10〜60分かかるため、1日数本という運用が現実的である。機密データ・未発表配列・患者情報をプロンプトに含めてはならない点は[§0-10 セキュリティと権限管理](#0-10-セキュリティと権限管理)および[§20 コードとデータのセキュリティ・倫理](./20_security_ethics.md)の原則と同じである。
...
>
> AIエージェントの能力を引き出すには、モデルへの指示（プロンプト）を工夫するだけでは足りない。エージェントが自律的に動く「環境」そのものを設計する必要がある。この技術分野は**ハーネスエンジニアリング**（Harness Engineering）と呼ばれ、2026年にMitchell Hashimoto（HashiCorp創業者）が命名して広まった。「ハーネス」は馬具の意味であり、強力だが手綱なしでは好き勝手に走る馬をたとえたものである。
>
> ハーネスエンジニアリングの核心は「**エージェントが間違いを犯すたびに、二度とその間違いを犯さない仕組みをエンジニアリングする**」ことにある。具体的には以下のプラクティスで構成される。
>
> | プラクティス | 内容 | 本書での対応 |
> |---|---|---|
> | 設定ファイルの累積的改善 | エージェントの失敗のたびにルールを追加する「生きたドキュメント」 | CLAUDE.md / AGENTS.md（本節） |
> | アーキテクチャ制約の機械的強制 | リンター、テスト、pre-commitフックで違反を自動検出 | [§8](./08_testing.md) |
> | 生成と評価の分離 | コードを書くエージェントと品質を判定するエージェントを分ける | [§0-4](#0-4-エージェントにレビューさせる) |
> | 段階的実装の強制 | 一括実装ではなく機能ごとに段階的に進め、各段階で検証する | [§0-2](#0-2-plan--execute--review-ワークフロー) |
>
> AIとの協働方法は、プロンプトエンジニアリング（「何と言うか」）からコンテキストエンジニアリング（「何を見せるか」）、そしてハーネスエンジニアリング（「どんな環境を作るか」）へと発展してきた。本書で学ぶ設定ファイル・テスト・リンター・Git運用の知識は、すべてハーネスの構成要素である。実験ノートに失敗を記録して次の実験に活かすように、エージェントとの協働でも失敗を設定ファイルやテストに変換して蓄積していくことが、信頼性の高い開発の鍵となる。

### エージェントの拡張機能 — MCP・フック・カスタムコマンド

設定ファイルは「エージェントへのルールの伝達」であった。AIコーディングエージェントには、さらにエージェントの**能力そのもの**を拡張したり、運用を自動化したりする仕組みが用意されている。以下の4つを知っておくと、本書を読み進める中でエージェント活用の幅が大きく広がる。ただし、利用できる機能はツールごとに異なる。

| 拡張機能 | 何ができるか | 本書での詳細 |
|---------|-------------|-------------|
| **MCP**（Model Context Protocol） | エージェントに外部ツールやデータベースへのアクセス能力を追加する。たとえばGitHub MCPを追加すると、エージェントがIssueの検索やPRの作成を直接行えるようになる | [§5-5](./05_software_components.md#5-5-mcpmodel-context-protocol-エージェントの能力を拡張する) |
| **フック**（Hooks） | 一部のエージェントでは、ツール実行の前後にシェルコマンドを自動実行できる。Claude Code では hooks により、編集後の lint や test の自動実行を設定できる | [§8-3](./08_testing.md#エージェントフック--ツール実行前後の自動チェック) |
...
上表の「ワークツリー分離」とは、同じリポジトリの作業コピーを一時的にもう1つ作り、メインの作業ディレクトリとは別の場所で変更を行う仕組み（`git worktree`）である。ブランチが「どの履歴の流れで作業するか」を切り替えるのに対し、ワークツリーは「作業場所そのものを増やす」点が異なる（詳しくは[§7-1](./07_git.md#ブランチと切り替え)を参照）。エージェントが並列実行時に内部で自動的に使うため、ユーザーが手動で操作する必要はない。
...
- 近年の上位モデルは、以前の世代と比べてサブエージェントを自発的には生成しにくい傾向がある[27](https://platform.claude.com/docs/en/about-claude/models/whats-new-claude-4-7)。並列化したいタスクは「以下の3つを並列のサブエージェントで」のように分割の粒度ごと明示的に指示する

### 具体的な活用場面

たとえば、新しいプロジェクトの立ち上げ時に以下のようなタスクを並列化できる:

- サブエージェント1: 既存の類似ツールを調査し、設計の参考にする（ただし最新のツール比較やベンチマーク論文を横断した大規模な文献サーベイは、CLI内サブエージェントより[§0-2 計画フェーズの外部調査 — Deep Researchの活用](#計画フェーズの外部調査--deep-researchの活用)で扱うDeep Researchのほうが向く）
...
コンテキストウィンドウとは、エージェントが一度に「記憶」できる情報量の上限を指す。2026年現在、200Kから1Mトークン程度が一般的である[5](https://docs.anthropic.com/en/docs/about-claude/models)。日本語の場合、1トークンはおおむね1〜2文字に相当するため、200Kトークンで数十万文字——書籍1冊分程度の情報を保持できる計算になる。
...
- **プロジェクト設定ファイルを活用する** — セッションをまたいで保持すべき情報は[§0-3](#0-3-プロジェクト設定ファイルclaudemd--agentsmd)で説明した設定ファイルに書く。設定ファイルは毎セッション自動で読み込まれるため、「忘れられない記憶」として機能する
...
この節が扱う具体的なモデル名・既定値・推奨値は、モデル世代の交代とともに変わる。そこで本文では**変わりにくい判断の軸**を説明し、変わりやすい具体値には最終確認日を添えた。読者が本書を読む時点での推奨は、各社の公式ドキュメント[1](https://code.claude.com/docs) [28](https://learn.chatgpt.com/docs/models) で確認してほしい。
...
> **モデル世代に依存する記述** — 最終確認: 2026-07-19。以下の具体値は世代交代で変わる。一次情報は Claude Code のモデル設定[29](https://code.claude.com/docs/en/model-config) と Codex のモデル一覧[28](https://learn.chatgpt.com/docs/models) を参照。
...
Codex CLI の GPT-5.6 は、Sol（最上位）・Terra（標準）・Luna（軽量）という3つのモデルに分かれている[28](https://learn.chatgpt.com/docs/models)。天体の名前が能力とコストの順に対応していると覚えるとよい。

起動時にどのモデルが選ばれるかは契約プランによって異なる。Claude Code CLI では Max 系の契約が Opus 4.8、Pro 系が Sonnet 5 で起動する[29](https://code.claude.com/docs/en/model-config)。「同じ本を読んでいるのに手元の既定が違う」ということが起こるので、最初に `/model` で現在の選択を確認しておくとよい。
...
第一に、**effort の段階名はモデルごとに較正されている**。同じ `high` でも、モデルが変われば実際に費やされるトークン量は変わる[29](https://code.claude.com/docs/en/model-config)。あるモデルで詰めた設定を別のモデルにそのまま持ち込んでも、同じ挙動にはならない。モデルを切り替えたら深さも調整し直す、と考えておきたい。

第二に、**すべてのモデルが effort に対応しているわけではない**。軽量モデルは非対応のことがあり、Claude Code CLI では指定してもエラーにならず、そのモデルが対応する最上位のレベルに黙って丸められる[29](https://code.claude.com/docs/en/model-config)。設定したつもりで効いていない、という状態が起こりうる。

> **モデル世代に依存する記述** — 最終確認: 2026-07-19。段階の名前・数・既定値はいずれも世代交代で変わる。

Claude Code CLI の effort は `low` / `medium` / `high` / `xhigh` / `max` の5段階で、既定は `high` である。`/effort` という専用コマンドで変更する。Haiku 4.5 は effort に対応していない[29](https://code.claude.com/docs/en/model-config)。Codex CLI は Low / Medium / High / Extra high / Max / Ultra の6段階で、既定は Medium。こちらは専用コマンドではなく、モデルを選ぶ `/model` の中で推論強度もあわせて指定する[2](https://github.com/openai/codex)。

| | Claude Code CLI | Codex CLI |
|--|-------------|-----------|
| 深さの段階 | `low`〜`max`（5段階）[29](https://code.claude.com/docs/en/model-config) | Low〜Ultra（6段階）[2](https://github.com/openai/codex) |
| 既定 | `high` | Medium |
...
Claude Code CLI には、そのターンだけ推論を深くする `ultrathink` というキーワードがある。プロンプトのどこかに書けば、セッションの effort 設定を変えずに済む。注意したいのは、**`think hard` や `think more` はキーワードとして認識されず、ただの文章として渡される**点である[29](https://code.claude.com/docs/en/model-config)。よく見かける言い回しだが、効果はない。

### 使い分けの指針 — 判断軸は「要件の曖昧さ」

上位のモデルや深い推論をいつ使うべきか。「タスクが難しいとき」という答えは、実のところ使いにくい。難しさは主観的で、着手する前には見積もれないことが多いからである。

両社の公式ドキュメントが示す判断軸は、難しさではなく**要件がどれだけ固まっているか**である。Codex CLI の説明が明快で、最上位は「曖昧で、難しく、追加の分析や判断を要するタスク」に、軽量は「**良い結果がどういうものか既に分かっている**、具体的で大量のタスク」に向くとされる[28](https://learn.chatgpt.com/docs/models)。
...
Claude Code CLI の最上位クラスである Fable 5 は、サイバーセキュリティと生物学の内容に対する安全性分類器とともに動作する。分類器が反応すると、Claude Code はそのリクエストを自動的に Opus 4.8 で実行し直し、切り替えたことを画面に表示する[29](https://code.claude.com/docs/en/model-config)。公式ドキュメントは、**生物学に隣接したコードベースでは頻繁に、多くの場合は最初のリクエストから切り替えが起き、本格的な生物学の作業ではほぼすべてのリクエストが再ルーティングされると予想せよ**と明記している[29](https://code.claude.com/docs/en/model-config)。

なぜ「最初のリクエストから」なのか。セッションの最初のリクエストには、[§0-3 プロジェクト設定ファイル](#0-3-プロジェクト設定ファイルclaudemd--agentsmd)で作った `CLAUDE.md` の内容や `git status` といったワークスペースの情報が同梱される。つまり**特別なことを何も尋ねていなくても、リポジトリに生物学の材料が含まれているだけで分類器が反応しうる**[29](https://code.claude.com/docs/en/model-config)。本書の読者がまさに置かれる状況である。

ここで大事なのは、**これが異常ではない**という点である。公式は「これらの領域では想定されたルーティングであり、アカウントに問題があるわけではない」と述べている[29](https://code.claude.com/docs/en/model-config)。切り替えの通知を見て慌てる必要はない。

そのうえで、知っておくべき挙動が3つある。

- 原因の切り分けには `claude --safe-mode` を使う。`CLAUDE.md`・スキル・MCP サーバ・フックを無効にして起動できる。ただし `git status` とディレクトリ名は無効化の対象外なので、これらが原因の場合は切り分けきれない[29](https://code.claude.com/docs/en/model-config)
- 自動で切り替えず毎回確認させたい場合は、`/config` で該当の設定をオフにする[29](https://code.claude.com/docs/en/model-config)
- **非対話モードや SDK 経由では、確認を表示できないため拒否でターンが終わる**[29](https://code.claude.com/docs/en/model-config)。`claude -p` を CI に組み込む場合はここに注意する
...
`ultracode` は API の effort レベルではない。最も深い effort を指定したうえで、Claude Code が複数エージェントによるワークフローを起動してよいという常時許可を与える設定である[25](https://platform.claude.com/docs/en/build-with-claude/effort)。Codex CLI の Ultra も同様に「単一エージェントの実行を超えるもの」と説明され、サブエージェントを使って作業を分担させる[28](https://learn.chatgpt.com/docs/models)。つまりこれらは推論を深くするつまみではなく、**仕事の進め方を並列に変えるスイッチ**である。

両社とも、**ほとんどのタスクにこれらは不要**だと明言している[28](https://learn.chatgpt.com/docs/models)。使いどころは「作業を意味のある単位に分割できるとき」に限られる。

ここで[§0-5 サブエージェントとタスク委譲](#0-5-サブエージェントとタスク委譲)の内容とつながる。**この最上位設定を選んでいない限り、並列の委譲は明示的に指示する必要がある**[28](https://learn.chatgpt.com/docs/models)。「以下の3つを並列のサブエージェントで調べて」のように、分割の粒度まで指定するのはこのためである。

もう一点、安全性に関わる注意がある。多数のエージェントを並列に走らせる設定と、承認なしで実行を許す権限モードの組み合わせは、影響範囲が一気に広がる。実際 Codex CLI では、この2つを同時に有効にしようとすると専用の警告が表示されるようになった[28](https://learn.chatgpt.com/docs/models)。権限の考え方は[§0-10 セキュリティと権限管理](#0-10-セキュリティと権限管理)で扱うが、**並列度を上げるときは権限を締める**、と覚えておきたい。

### 新しいモデルが出たときの作法

モデルは数か月おきに世代交代する。そのたびに設定を見直すことになるが、ここには**直感と逆の作法**がある。

新しいモデルが出たら、まず**推論の深さを一段下げて試す**。両社の公式移行ガイドが、揃ってこれを勧めている。Anthropic は最新世代について「低い effort の設定でも十分に機能し、旧世代の高い effort を上回ることが多い」と述べ[25](https://platform.claude.com/docs/en/build-with-claude/effort)、OpenAI は「現在の設定を基準として保ち、そこから一段下げたものと代表的なタスクで比較せよ。より少ないトークンで同等以上の品質を保てることが多い」と指示している[30](https://developers.openai.com/api/docs/guides/latest-model)。
...
サブスクリプション契約には利用枠があり、一定時間ごと（たとえば5時間のローリング枠）と週単位で上限が設定されている[29](https://code.claude.com/docs/en/model-config)。しかも**この枠はモデルをまたいで共有される**。上位モデルの枠を使い切ったとき、`/model` で軽量モデルに切り替えても枠そのものは回復しない[29](https://code.claude.com/docs/en/model-config)。つまり「サブスクだから気にしなくてよい」のではなく、枠の中でどう配分するかを考える必要がある。

コストを左右する要因は、モデルの選択と推論の深さの2つである。上位モデルほど、また深い推論ほどトークンを多く使う。ここで見落としやすいのが、**思考のために生成されるトークンも課金対象**だという点である。画面に折りたたまれて見えていなくても、深く考えさせればその分だけ費用がかかる[29](https://code.claude.com/docs/en/model-config)。

節約の基本は「すべてに最上位を使わない」ことに尽きる。公式ドキュメントも、大半のコーディングは標準モデルで十分にこなせるとし、上位モデルは複雑な設計判断や多段の推論に取っておくよう勧めている[29](https://code.claude.com/docs/en/model-config)。深い推論が要らない定型作業では、`/effort` で深さを下げるだけでも消費を抑えられる。
...
> | 「一時的に回避しておきます」「ワークアラウンドを追加します」 | **バンドエイドフィックス** — 根本原因を特定せず、症状だけを抑える応急処置を積み重ねる | エージェントに「この修正は根本的な解決か、一時しのぎか」を問う。応急処置が増えたら設計を見直す |
> | 修正A→修正B→また修正Aに戻る | **オシレーション** — 相反する方針の間で修正が往復し、進捗しない | 振り子の兆候が見えたら立ち止まり、人間がどちらの方針を採るか決定する。過去にトレードオフを受け入れた判断をエージェントが覆そうとしていないかも確認する |
> | 「念のためこのケースも…」「後方互換性のために…」 | **過剰レビュー・過剰指摘** — 実際には起こりえないエッジケースの指摘、不要な後方互換性の要求、既に受け入れたトレードオフの蒸し返し | 指摘の前提を確認する。「そのケースは本当に起こりうるか？」「その互換性は誰が必要としているか？」と問い返す |
>
> これらに共通するのは、「AIが安全側・汎用側に倒そうとする」傾向である。研究コードでは多くの場合、シンプルさのほうが価値がある。
>
> なお近年の上位モデルでは、これらのアンチパターンに見られる**過剰さ**自体がモデル側でも抑制される傾向にある[27](https://platform.claude.com/docs/en/about-claude/models/whats-new-claude-4-7)。一方で低い effort では「頼まれた範囲」のみに厳密に絞り込むため、必要な防御や検証はプロンプトに**明示列挙**する必要がある。「過剰を削る」と「必要なものを書き落とす」は表裏なので、生成されたコードに何が含まれていないかを意識してレビューしたい。
...
AIコーディングエージェントはローカル環境でファイルの読み書きやコマンドの実行を行うため、セキュリティへの配慮が不可欠である。本節ではまずローカル操作の権限管理を整理し、続いてエージェントに**送信する**データそのものに対する注意——プライバシーと未発表データの取り扱い——を述べる。
...
- **自動実行設定は信頼できるタスクでのみ使う** — 初心者は人が確認しながら編集する設定を基本にする
...
### 0-10-2. プライバシーと未発表データの取り扱い

権限ホワイトリストやサンドボックスは「エージェントがローカル環境に対して何をしてよいか」を制御する仕組みである。一方で、エージェントとの対話そのものによってコードや実験データが**プロバイダ側のサーバに送信される**点には別の注意が必要である。セットアップと権限設計の段階で、次の事実を押さえておく必要がある。

研究現場でエージェントに情報を渡す際のリスクは、大きく二つに分けられる。

- **規約違反のリスク**: 制限付きデータベース（dbGaP・JGA・EGA 等）から取得したデータには Data Use Agreement（DUA; データ利用規約）がかかっており、その多くは承認されたコンピューティング環境外への転送を禁じている。クラウドで動作する AI エージェントのサーバは通常この「承認された環境」に含まれない
- **研究倫理・知財のリスク**: 査読中の原稿、未公開の新規配列、特許出願前の発明、共同研究者から受け取った未発表素材、NDA/MTAの対象物などは、規制対象の個人情報でなくとも、外部への送信が研究倫理・共同研究契約・知財保護の観点から問題になりうる

エージェントに情報を渡す前に、最低限、次の3ステップで仕分けをする習慣を持つとよい。

1. **公開前提か非公開か**: その情報が将来的に公開される予定のものか（例: 解析結果を論文公開する予定）、それとも非公開を前提としたものか（例: 他人の未発表素材、契約対象物）を区別する
2. **契約・規約の対象か**: DUA・NDA・MTA・共同研究契約・所属機関の情報管理規程の対象であれば、条文を確認する。不明な場合は送信しない
3. **ローカル実行を検討するか**: 対象であれば、原則としてクラウド AI エージェントには渡さず、ローカルで動作する LLM エージェントの利用を検討する（詳細と選択肢は[§20-2-1 AI エージェントに渡してよいデータの判断](./20_security_ethics.md#ai-エージェントに渡してよいデータの判断)を参照）

この仕分けは、CLI エージェントへの入力だけでなく、[§0-2 計画フェーズの外部調査 — Deep Researchの活用](#計画フェーズの外部調査--deep-researchの活用)で扱うブラウザ側の Deep Research サービスにも同様に適用される。DR はレポート生成のために大量の情報を外部プロバイダのサーバ上で処理するため、制限付きデータ・未発表配列・患者情報・共同研究者から受け取った未公開素材などをプロンプトやアップロードファイルに含めてはならない。

#### プロバイダのプライバシー設定は各自が確認する
...
以下は、プライバシー設定や未発表データの仕分けをエージェント自身に手伝わせる指示例である。ただし、どの指示もプロジェクトの中身（配列データ、原稿、契約条文）をそのまま貼り付けるのではなく、ファイル名や概要だけを伝える形で始めるのが安全である。

> 「現在のサインイン情報（Consumer アカウントか API か Enterprise 契約か）を確認する手順を教えて。それぞれのプランでこのツールの入力データがモデルの学習に使われる扱いが既定でどうなっているかも、公式ドキュメントの該当ページへのリンク付きで整理して」

> 「このリポジトリの `data/` 配下にあるファイル名だけを一覧して。ファイル名から、DUA 対象のデータや未公開配列が含まれていそうなものを判別し、エージェントに直接中身を読み込ませるべきでないファイルを候補としてリストアップして。中身の読み込みはまだしなくてよい」

> 「`.gitignore` と `CLAUDE.md` を見直し、未発表原稿（`manuscripts/`）、制限付きデータ（`data/controlled/`）、共同研究者から受け取った素材（`collab/`）のいずれもエージェントから自動的に読まれない設定になっているか確認して。不足があれば修正案を提示して」
...
1. **承認ポリシーとサンドボックスを理解する**（[§0-1](#0-1-セットアップと基本操作)）— 初心者は人が確認しながら編集する設定を基本に
2. **Plan → Execute → Reviewのワークフローを守る**（[§0-2](#0-2-plan--execute--review-ワークフロー)）— いきなり実装させない
3. **プロジェクト設定ファイルを整備する**（[§0-3](#0-3-プロジェクト設定ファイルclaudemd--agentsmd)）— エージェントへの「取扱説明書」
...
[§0-3](#0-3-プロジェクト設定ファイルclaudemd--agentsmd)で学んだプロジェクト設定ファイルの考え方に基づき、RNA-seq解析パイプラインのプロジェクトを想定したCLAUDE.mdを作成せよ。以下の要素を含めること:
...
- [5](https://docs.anthropic.com/en/docs/about-claude/models) Anthropic. "Models overview". https://docs.anthropic.com/en/docs/about-claude/models (参照日: 2026-03-17)
...
- [14](https://arxiv.org/abs/2601.11868) Wijk, D., Phan, L., Berglund, L., et al. "Terminal-Bench: Benchmarking Agents on Hard, Realistic Tasks in Command Line Interfaces". *ICLR 2026*. https://arxiv.org/abs/2601.11868
...
- [17](https://pubmed.ncbi.nlm.nih.gov/38940140/) Tang, X., Jiang, B., Zhuo, Y., Cheng, Y., Phung, D., Gerstein, M. "BioCoder: a benchmark for bioinformatics code generation with large language models". *Bioinformatics*, 40(4), 2024. https://pubmed.ncbi.nlm.nih.gov/38940140/
...
- [22](https://arxiv.org/abs/2508.05668) Huang, Y., et al. "A Survey of LLM-based Deep Search Agents: Paradigm, Optimization, Evaluation, and Challenges". *arXiv preprint*, 2025. https://arxiv.org/abs/2508.05668
- [23](https://arxiv.org/abs/2508.12752) Xu, R., et al. "Deep Research: A Survey of Autonomous Research Agents". *arXiv preprint*, 2025. https://arxiv.org/abs/2508.12752
...
- [27](https://platform.claude.com/docs/en/about-claude/models/whats-new-claude-4-7) Anthropic. "What's new in Claude Opus 4.7". https://platform.claude.com/docs/en/about-claude/models/whats-new-claude-4-7 (参照日: 2026-04-25)
- [28](https://learn.chatgpt.com/docs/models) OpenAI. "Models — Codex". https://learn.chatgpt.com/docs/models (参照日: 2026-07-19)
- [29](https://code.claude.com/docs/en/model-config) Anthropic. "Model configuration". https://code.claude.com/docs/en/model-config (参照日: 2026-07-19)
- [30](https://developers.openai.com/api/docs/guides/latest-model) OpenAI. "Migrate to the latest model". https://developers.openai.com/api/docs/guides/latest-model (参照日: 2026-07-19)
