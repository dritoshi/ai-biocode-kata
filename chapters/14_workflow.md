...
> **この章の前提知識**: シェルスクリプト（`for`文、変数展開）を読めること。不安な場合は [§2 ターミナルとシェルの基本操作](./02_terminal.md) を復習し、[Linux標準教科書](https://linuc.org/textbooks/linux/) を参照するとよい。
...
この問題を解決するのが**ワークフロー言語**（workflow language）である。ワークフロー言語は処理ステップ間の依存関係を宣言的に記述し、実行順序の決定・並列化・途中からの再開をエンジンに委ねる仕組みである。これは[§1-1 設計原則とプログラミングパラダイム](./01_design.md#設計原則とプログラミングパラダイム)で紹介した宣言型の考え方を、解析パイプラインへ応用した例である。ただし、言語によって宣言型の度合いは異なり、手続き的な処理を埋め込めるものもある。代表的なツールとしてSnakemake、Nextflow、CWL、makeがあり、いずれもバイオインフォマティクスのパイプライン構築で広く使われている。
...
Nextflow[2](https://doi.org/10.1038/nbt.3820)[5](https://docs.seqera.io/nextflow/)はDSL2構文を採用したワークフロー言語で、データの流れを**チャネル**（channel）として表現する。データの到着と依存関係を中心に処理を組み立てるこの考え方を**データフロー型**という。宣言型と親和性は高いが同義ではなく、Nextflow DSL2には手続き的に記述できる部分もある。Snakemakeの「ファイルベース」の依存解決とは設計思想が異なる。
...
> <a id="column-ch14-01"></a> 🧬 **コラム: QCメトリクスの読み方**
>
> 数値の「合格ライン」を知る。
>
...
> <a id="column-ch14-02"></a> 🧬 **コラム: nf-coreエコシステム**
>
...
[§1-1 コードの設計原則](./01_design.md#実行条件への応用--設定を外から与える)で学んだ、実行環境やデータセットによって変わる値をコードから分離する考え方は、ワークフローにも当てはまる。パラメータをSnakefileに直書きすると、異なる条件での再実行時にワークフロー定義自体を編集することになり、変更の追跡が困難になる。
...
> <a id="column-ch14-03"></a> 🤖 **コラム: 機械学習パイプラインのワークフロー管理**
>
...
以下の4つのシナリオについて、どのワークフロー言語（Snakemake / Nextflow / CWL）で管理すべきか、あるいはワークフロー言語を使わないべきかを判断し、その理由を述べよ。

- (a) 3ステップ、1サンプルの解析を1回だけ実行する
- (b) 10ステップ、20サンプルの解析を実行する
- (c) 3ステップ、1サンプルの解析だが、月次で繰り返し実行する
- (d) dbGaPの制限付きデータをTerra/AnVIL上で解析する必要がある

（ヒント）(a)〜(c)はステップ数×サンプル数の積と繰り返し頻度が判断基準である。(d)ではTRE環境が求めるワークフロー形式（CWL/WDL）も判断材料になる。
