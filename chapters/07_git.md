...
ここで重要な区別を明確にしておこう。**GitとGitHubは別のものである**。Gitは Linus Torvalds が2005年に開発した**バージョン管理ソフトウェア**であり、インターネット接続なしにローカルPCだけで動作する。§7-1で学んだ `git init`、`git add`、`git commit` はすべてローカルで完結する操作である。一方、**GitHub**はMicrosoft が運営する**Webサービス**であり、Gitで管理したリポジトリをクラウド上にホスティングし、共有・公開・共同開発のための機能（Issue、Pull Request、Actions等）を提供する。GitHubの代替としてGitLab、Bitbucketなどのサービスも存在する。「GitHubを使う」ことと「Gitを使う」ことは同義ではない——Gitは道具、GitHubはその道具を活かすための場所である。
