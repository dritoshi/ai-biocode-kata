...
> — ヤン・ウェンリー, 田中芳樹『銀河英雄伝説』第1巻 黎明篇 (徳間書店, 1982)
...
![Gitの3ステージモデル: 作業ディレクトリ → ステージングエリア → リポジトリ](../figures/ch07_git_three_stages.png)
...
**ブランチ**（branch）は、本体のコードに影響を与えずに実験的な変更を試せる仕組みである。幹（main）から枝（branch）を伸ばし、うまくいったら幹に統合（merge）するイメージである。近年、Gitでは従来の `git checkout` に代わり、役割を明文化した `git switch`（ブランチ切り替え）と `git restore`（ファイルの復元）の使用が推奨されている。

```bash
# 新しいブランチを作成して切り替える
git switch -c add-gc-filter
...
![ブランチとマージの概念: mainから分岐して作業し、完了後にマージで統合する](../figures/ch07_branch_merge.png)

研究のプログラミングでは、「新しい解析手法を試す」「パラメータを変えて比較する」といった場面でブランチが役立つ。mainブランチは常に動作する状態を保ち、実験的な変更はブランチで行う。

なお、Gitには**ワークツリー**（`git worktree`）という仕組みもある。ブランチは「どの履歴の流れで作業するか」を切り替えるが、作業ディレクトリは1つのままである。ワークツリーは同じリポジトリの**別のブランチを別のディレクトリに展開**し、2つの作業場所を同時に持てる。AIコーディングエージェントはサブエージェントの並列実行時にこの仕組みを内部で利用する（[§0-5 サブエージェントとタスク委譲](./00_ai_agent.md#0-5-サブエージェントとタスク委譲)参照）。ユーザーが手動で `git worktree` を使う場面はほとんどないが、エージェントのログに `worktree` という単語が出てきたときに「ああ、並列実行用に作業場所を分けたのだな」と理解できれば十分である。
...
§7-1で学んだGitの操作はすべてローカルPCで完結する。コードを他者と共有し、バックアップを取るには、Gitリポジトリをクラウド上に置く**リモートリポジトリ**が必要になる。GitHubはその最も広く使われるホスティングサービスであり、GitLabやBitbucketなどの代替サービスも存在する。
...
> <a id="column-ch07-01"></a> **🧬 コラム：バイオインフォツール公開の実例**
>
...
`git lfs install` はGit LFSのフィルタを現在のユーザー環境（または `--local` 指定時はそのリポジトリ）に設定するコマンドで、通常は初回のみ実行する。`git lfs track` は指定パターンのファイルをLFS管理対象として `.gitattributes` に登録する。`.gitattributes` は他の共同研究者も同じLFS設定を使えるようにGitで管理する必要がある。
...
GitHub Docs によれば、GitHub Free / Pro / 組織向け GitHub Free の Git LFS 無償枠はストレージ 10 GiB・月間帯域 10 GiB であり、Team / Enterprise Cloud は 250 GiB である[17](https://docs.github.com/ja/billing/concepts/product-billing/git-lfs)。バイオインフォマティクスでは参照ゲノム、シングルセル解析の `.h5ad` ファイル、学習済みモデルなどが容易にこの制限を超える。そのような大規模データには、用途に応じた外部サービスを組み合わせるのが現実的である。

| データの種類 | 推奨サービス | 理由 |
|-------------|------------|------|
| 学習済みモデル（数百MB〜数十GB） | Hugging Face Hub | モデルカード・バージョン管理・ストリーミング対応 |
| 論文付随データ（数十GB級） | Zenodo | DOI 発行可。論文引用に最適 |
| 大規模リファレンス・共有データ | S3, Box, Dropbox | 機関提供ストレージ。アクセス制御が柔軟 |

**Hugging Face Hub** [16](https://huggingface.co/docs/hub/) は、Git ベースのホスティングで `git lfs` と同じ操作感を持つ。`huggingface_hub` Python ライブラリを使えば、スクリプトからプログラマティックにデータをアップロード・ダウンロードできる。機械学習モデルだけでなく、データセットのホスティングにも広く使われている。

**Zenodo** は§7-2で紹介したコードの DOI 発行だけでなく、データセットの永続保存にも使える。標準では 1 レコードあたり 50GB・100ファイルまでで、必要に応じて 200GB までの増量を申請できる[18](https://help.zenodo.org/docs/deposit/manage-files/)。論文付随データの公開に適している。
...
### 演習 7-1: コミットメッセージの改善 **[概念]**

以下の3つのコミットメッセージを、Conventional Commits 形式に書き直せ。それぞれ「なぜその変更を行ったか」が伝わるようにすること。

- (a)「ファイルを直した」
- (b)「新しいやつ追加」
- (c)「いろいろ変更」

（ヒント）`fix:`, `feat:`, `refactor:` などの型プレフィックスを使い、「なぜ」を含めた説明を書く。変更対象のスコープが分かる場合は `fix(parser):` のようにスコープも付与する。

### 演習 7-2: .gitignore の確認 **[レビュー]**

エージェントが生成した `.gitignore` に以下の項目が含まれていない。追加すべきものをすべて選び、それぞれの理由を述べよ。
...
（ヒント）秘密情報、大規模データ、ビルド成果物（再生成可能なファイル）、OS 固有ファイルの4カテゴリに分けて考えると整理しやすい。`results/` については、再現可能かどうかで判断が分かれる点にも注意する。

### 演習 7-3: ライセンスの選択 **[設計判断]**

自作のバリアントコーラーをオープンソースとして公開したい。MIT, Apache 2.0, GPL-3.0 の3つから1つを選び、選択理由を述べよ。さらに、GPL-3.0 ライブラリに依存している場合、ライセンス選択にどのような制約が生じるかを説明せよ。

（ヒント）GPL の「コピーレフト」の影響範囲を確認する。GPL ライブラリをリンク・インポートして利用するソフトウェアは、そのソフトウェア自体も GPL 互換のライセンスで公開する必要がある。

### 演習 7-4: CITATION.cff の生成 **[実践]**

エージェントに自分のツールの `CITATION.cff` を生成させ、以下の必須フィールドが正しく記入されているかレビューせよ。

- `title`
- `authors`（`given-names`, `family-names`, `orcid`）
- `version`
- `date-released`

（ヒント）`doi` フィールドは GitHub Release + Zenodo 連携後に埋まるため、初回生成時は空でよい。ORCID は `https://orcid.org/` で始まる URL 形式であることを確認する。

---

## さらに学びたい読者へ

本章で扱ったGit/GitHubの基礎をさらに深く学びたい読者に向けて、教科書とオンラインリソースを紹介する。
...
- **GitHub Skills.** https://skills.github.com/ — GitHubが提供するインタラクティブな学習コース。ブランチ、Pull Request、GitHub Actions等を実際のリポジトリで練習できる。
