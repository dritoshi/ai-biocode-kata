...
本書は文法の入門書ではない。AIエージェントがコードを生成する時代に、そのコードをレビューし判断するための「作法」を教える本である。本書のコマンド例・動作確認は macOS および Linux（WSL を含む）環境を前提としている。Windows ユーザーは WSL 2 上での作業を推奨する。以下の知識は本書の中では詳しく扱わないため、不安がある場合は先に学んでおくことを勧める。
...
- **坊農秀雅『生命科学研究のためのデジタルツール入門 第2版 — 結果に差がつく使いこなし術』**（メディカル・サイエンス・インターナショナル, 2024）[30](https://www.amazon.co.jp/dp/4815731063) — 生命科学のデータベースとウェブツールの使い方を統合TVの動画と連携して解説。初心者に最適
...
**プライバシー設定と規約は公式ドキュメントで必ず確認すること**。各プロバイダの最新のプライバシーポリシー・利用規約・データ利用ポリシーは、読者自身が利用開始前、および少なくとも年に一度は公式ドキュメントで確認してほしい。2026年4月時点の整理と公式ドキュメントへのリンクは[§20-2-1 AI エージェントに渡してよいデータの判断](./20_security_ethics.md#ai-エージェントに渡してよいデータの判断)に掲載しているが、そこに記載した内容も執筆時点の理解にすぎない。
...
研究室のラボメンバーみなさまに感謝する。特に、実験系生物科学者の立場から本書の企画に刺激を与えてくれた笹川洋平准教授と矢野実氏には、本書を書くきっかけそのものをいただいた。また、実験系のラボメンバーや共同研究者が日々の研究で本当に必要としていることは何かを常に意識しながら執筆を進めた。
