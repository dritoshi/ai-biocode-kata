...
> <a id="column-ch20-01"></a> **🧬 コラム: プロンプトインジェクションとバイオインフォマティクス**
>
...
**本節以降のエージェント指示例はすべて、ローカル環境で動作するエージェントの使用を前提としている。** 制限付きヒトデータを扱う場合、クラウドベースのAIサービスにデータを送信してはならない（[AI エージェントに渡してよいデータの判断](#ai-エージェントに渡してよいデータの判断)を参照）。
...
**原則: 制限付きヒトデータをクラウドAIサービスに送信してはならない。** 所属機関の情報セキュリティ部門とDACの双方から明示的な承認を得ている場合を除き、クラウドベースのエージェントに制限付きデータを渡すことは絶対に避けるべきである。制限付きデータを扱うコード生成・データ処理には、後述のローカルLLMエージェントを使用する。

##### ヒトデータ以外にも AI エージェントに渡すべきでない情報がある

規制対象のヒトデータだけが「送ってはならない情報」ではない。研究現場では、個人情報に該当しなくとも、契約・研究倫理・知財の観点からクラウドAIエージェントへの送信が問題になる素材が数多く存在する。代表例を以下に列挙する。

- **査読中の原稿本文・図表・レビュー対応資料**: 査読過程は通常、関係者以外への情報共有を禁じている。原稿をそのまま貼り付けて添削させる行為は、雑誌の機密保持ポリシーに抵触しうる
- **未公開の新規配列・ゲノムアセンブリ**: 論文投稿前や公的DBでの embargo 期間中の配列は、先取権（priority）と科学的貢献の帰属に直結する。embargo の趣旨はアクセッション番号付与後の先出し公開の抑制であり、外部AIへの送信は想定されていない
- **特許出願前の発明・新規アルゴリズム・手法の詳細**: 特許法上の新規性は「公然知られていないこと」を要件とする。クラウドAIへの送信が「公知」に該当するかは各国法と契約で議論中であり、知財戦略上は送信を避けるのが安全である
- **共同研究者から受け取った未発表素材**: プライベートに共有された BAM、未公開アノテーション、スクリーニング結果などは、第三者のデータである。共同研究者の明示的な同意なく外部サービスに渡すことはできない
...
AI コーディングエージェントのプライバシー設定は、**サインインに使うアカウントの種類と契約プランによって大きく異なる**。同じツールでも、個人の Consumer アカウントで使う場合と、組織の Enterprise 契約で使う場合とで、送信データの扱いは別物になる。以下に、契約の種類ごとの既定の扱いを 2026年7月時点の著者の理解に基づいて整理する。個別ツールの opt-out 手順は後述の「設定変更の実務手順」にまとめた。

> **重要な但し書き**: **本表は 2026年7月時点で著者が各社の公式ドキュメントを参照して整理したものであり、内容は頻繁に更新される。実際の運用に先立って、本節末尾の公式リンクから必ず現行ポリシーを確認すること**。所属機関の情報セキュリティ部門・倫理審査委員会・法務部門への事前相談も推奨される。
...
これらのページは頻繁に更新されるため、**ブックマークに入れておき、定期的に、あるいはツールやプランの契約変更時に必ず再確認する**ことを勧める。
...
このフローで「送信しない」と判断されたデータを扱う場合は、次項の**ローカルLLMエージェント**が選択肢となる。
...
> <a id="column-ch20-02"></a> 🧬 **コラム: Trusted Research Environment — 制限付きデータのクラウド解析基盤**
>
...
>
> TREは、制限付きデータを承認された研究者だけがアクセスできるクラウド上のセキュアな解析環境である。データはTREの外に持ち出せず、解析結果のみが審査を経て持ち出される。[§16](./16_hpc.md#column-ch16-05)のクラウドコラムで紹介したIaaS/PaaSの上に、アクセス制御・監査ログ・データ流出防止の層を追加した仕組みと考えればよい。
>
...
>
> これらのプラットフォームでは、データをダウンロードするのではなく、**解析をデータのある場所に持っていく**（[§16](./16_hpc.md#column-ch16-05)のクラウドコラム参照）。ワークフロー定義ファイル（WDL、CWL、Nextflow）やDockerコンテナをアップロードし、クラウド上で実行する。[§14](./14_workflow.md)で学んだワークフロー管理と[§15](./15_container.md)で学んだコンテナ技術の知識が、ここで直結する。

---

> <a id="column-ch20-03"></a> **🧬 コラム: dbGaPの利用規約違反とその教訓**
>
...
> <a id="column-ch20-04"></a> **🧬 コラム: ゲノムデータは究極の個人情報**
>
...
[27] Anthropic. "Privacy Policy". https://www.anthropic.com/legal/privacy (参照日: 2026-04-10)

[28] Anthropic. "Commercial Terms of Service". https://www.anthropic.com/legal/commercial-terms (参照日: 2026-04-10)

[29] OpenAI. "Enterprise Privacy at OpenAI". https://openai.com/enterprise-privacy/ (参照日: 2026-04-10)
