...
本章では、まず[20-1節](#20-1-セキュリティの基礎)でシークレット管理・エージェントのセキュリティ設定・依存パッケージの安全性を扱い、次に[20-2節](#20-2-データの倫理)でヒトデータの法規制・匿名化・研究倫理を学ぶ。
...
| **GDPR**[15](https://www.edpb.europa.eu/our-work-tools/documents/our-documents/opinion-282024-certain-data-protection-aspects-related_en) | EU | 遺伝データは「特別カテゴリーの個人データ」。EDPB は 2024年12月18日に Opinion 28/2024 を採択し、AIモデル文脈での匿名性、正当利益、違法取得データの扱いを整理した。AI 関連規制との関係は別途確認が必要 |
...
**データ管理計画**（Data Management Plan; DMP）は、研究プロジェクトで生成・取得するデータの管理方針を文書化したものである。JSPS の科研費では、令和6（2024）年度以降に実施する課題について DMP を活用した研究データ管理が求められている[13](https://www.jsps.go.jp/j-grantsinaid/01_seido/10_datamanagement/index.html)。研究開始前の DMP 提出自体は不要だが、各年度の研究終了後には、公開した研究データの情報（メタデータ等）を実施状況報告書・実績報告書の一部として提出する運用になっている。
...
以下のデータを解析する際に、クラウドAI（API経由）を使えるか判断せよ。それぞれ理由を述べよ。
...
（ヒント）DUA（Data Use Agreement）の「第三者への転送禁止」条項を確認せよ。クラウドAPIへのデータ送信は「転送」に該当しうる。公開データであっても、利用規約を確認する習慣が重要である。
...
[15] European Data Protection Board. "Opinion 28/2024 on certain data protection aspects related to the processing of personal data in the context of AI models". https://www.edpb.europa.eu/our-work-tools/documents/our-documents/opinion-282024-certain-data-protection-aspects-related_en (参照日: 2026-03-25)
