...
GenBank（NCBI）、ENA（EBI）、DDBJ（日本）は**国際塩基配列データベース連携**（INSDC）の三極体制で相互ミラーリングしている[6](https://doi.org/10.1093/nar/gkad1044)。どのDBに登録してもデータは三者間で共有される。なお、`NM_` `NR_` `XM_` などの下線付きアクセッションは RefSeq 系列であり、GenBank のアクセッションとは区別する。
