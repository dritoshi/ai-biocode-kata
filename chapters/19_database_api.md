...
> （情報は自由になりたがる。同時に、情報は高価でありたがる。この緊張は消えない。）
> — Stewart Brand, 第1回 Hackers Conference (1984)

> **この章の前提知識**: 公共データベース（NCBI、UniProt等）をWebブラウザで使ったことがあること。不安な場合は [統合TV](https://togotv.dbcls.jp/) や坊農秀雅『生命科学データベース・ウェブツール』を参照。
...
本章では、まず[19-1節](#19-1-データベースの基礎知識--エージェントと同じ言語で話すために)でDB種類・フラットファイル・クエリ言語・オントロジーの基礎語彙を整理する。次に[19-2節](#19-2-バイオインフォマティクスの主要データベース)で生命科学分野の主要データベースとID体系を概観する。[19-3節](#19-3-apiによるデータ取得)ではAPIを用いたプログラムからのデータ取得を実践し、最後に[19-4節](#19-4-データのダウンロードとローカル管理)で大規模データのダウンロードとローカル管理の手法——クラウドストレージ（S3/GCS）からの直接取得を含む——を学ぶ。

---

## 19-1. データベースの基礎知識 — エージェントと「同じ言語」で話すために
...
主要なRDB製品として、PostgreSQL、MySQL、SQLiteがある。SQLiteはサーバ不要でファイル1つに格納されるため、個人の解析プロジェクトでメタデータを管理するのに適している（[19-4節](#19-4-データのダウンロードとローカル管理)で実践する）。
...
実際の解析ではこの2つを組み合わせて使うことが多い。たとえば、ヒトゲノムリファレンス配列はFTPサイトからダウンロードし、特定の遺伝子のアノテーション情報はAPIで検索する、という使い分けである。[19-3節](#19-3-apiによるデータ取得)でAPI方式を、[19-4節](#19-4-データのダウンロードとローカル管理)でダウンロード方式をそれぞれ詳しく扱う。
...
2. **SPARQL**: RDFデータを提供するエンドポイント（UniProt等）に対するクエリ
3. **REST API**: HTTP経由でJSON/XML/FASTAを返すエンドポイント（NCBI E-utilities、TogoWS等）に対するリクエスト
...
> <a id="column-ch19-01"></a> **🧬 コラム: Gene OntologyとKEGG**
>
...
>
> **KEGG**（Kyoto Encyclopedia of Genes and Genomes）[5](https://pubmed.ncbi.nlm.nih.gov/10592173/)は、代謝パスウェイや疾患パスウェイなどの経路情報を体系化したデータベースである。KEGG REST APIでプログラムからパスウェイ情報を取得できるが、ライセンスに注意が必要である——学術目的の利用は無料だが、商用利用には有料ライセンスが必要である。
>
...
|----------|------|---------------|-------------|
| NCBI系 | GenBank | 塩基配列 | U12345, AF123456 など |
...
GenBank（NCBI）、ENA（EBI）、DDBJ（日本）は**国際塩基配列データベース連携**（INSDC）の三極体制で相互ミラーリングしている[6](https://pubmed.ncbi.nlm.nih.gov/37994677/)。どのDBに登録してもデータは三者間で共有される。なお、`NM_` `NR_` `XM_` などの下線付きアクセッションは RefSeq 系列であり、GenBank のアクセッションとは区別する。

### DB選択の判断力

データの種類に応じて適切なDBを選択することは、エージェントに指示する前に人間が行うべき判断である。以下のフローで考えると整理しやすい。

![DB選択フローチャート: データの種類に応じた公共データベースの選択](../figures/ch19_db_selection.png)
...
リクエストとレスポンスの形式として最も一般的なのが**JSON**（JavaScript Object Notation）である。JSONの構造については[§4-2](./04_data_formats.md)で学んだ。APIのレスポンスとして返されるJSONをPythonの辞書やリストに変換して処理するのが、API利用の基本パターンである。
...
**エンドポイント**とは、APIが公開するURLのことである。たとえばNCBI E-utilitiesでは `https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi` のようなエンドポイントが使われる。
...
> <a id="column-ch19-02"></a> **🧬 コラム: NCBI系コマンドラインツール**
>
...
> <a id="column-ch19-03"></a> **🧬 コラム: Entrez以外の主要バイオインフォマティクスAPI**
>
...
> <a id="column-ch19-04"></a> 🧬 **コラム: バイオインフォマティクス向けMCPサーバーとAIエージェント**
>
...
#### クラウドストレージからの直接取得（S3 / GCS）

NCBIはシーケンスデータをAWS S3およびGoogle Cloud Storage（GCS）でも公開している。SRA Toolkitの`prefetch`やAsperaを使わず、クラウドのCLIで直接ダウンロードできる。特にクラウド上で解析する場合は、同一リージョン内のデータ転送が高速かつ無料になるため、S3/GCS経由が有利である。

```bash
# AWS S3 からSRAデータを直接ダウンロード（認証不要）
...
エージェントにダウンロードスクリプトの一括生成を依頼するのは有効なパターンである。アクセッション番号のリストを渡して「50件をprefetch + fasterq-dumpで並列ダウンロードするbashスクリプトを書いて」のように指示すれば、ループ処理やリトライロジックを含むスクリプトを生成してくれる。クラウド環境であれば「S3から直接取得するスクリプトに変更して」と追加指示を出せばよい。
...
[19-1節](#19-1-データベースの基礎知識--エージェントと同じ言語で話すために)で紹介したように、GenBank形式はFASTAと異なりfeature情報（CDS、gene、mRNA等のアノテーション）を含む。配列だけでなく遺伝子の座標や機能アノテーションが必要な場合はGenBank形式を選ぶ。
...
> <a id="column-ch19-05"></a> **🧬 コラム: データベースへの登録 — Vibe codingで登録準備を効率化する**
>
...
> | 論文データセット（キュレーション付き） | Dryad | 300GBまで。査読連携あり |
> | 機関要件がある場合 | 機関リポジトリ | 科研費のメタデータ報告先として推奨 |
>
> 科研費では2025年度から、公開した研究データのメタデータ報告が実績報告書で義務化されている。データ本体は上記のいずれかのリポジトリに格納し、メタデータにDOIまたは公開URLを記載する。詳細は[§20 コードとデータのセキュリティ・倫理](./20_security_ethics.md)のDMPセクションを参照。
>
...
### 演習 19-1: データベースの使い分け **[概念]**

以下の情報を取得するとき、それぞれどのデータベースを使うべきか答えよ。選択の理由も述べよ。

1. ヒトのタンパク質配列
2. RNA-seqの生データ（FASTQファイル）
3. 遺伝子オントロジー（GO）のアノテーション

（ヒント）(1)UniProt (2)SRA/GEO (3)Gene Ontology / Ensembl BioMart。一次データベースと二次データベースの役割の違いを思い出すこと。

### 演習 19-2: API利用コードのレビュー **[レビュー]**

以下のNCBI Entrez APIを使った配列取得コードをレビューし、問題点をすべて指摘せよ。

```python
from Bio import Entrez

def fetch_sequences(gene_list):
    results = []
    for gene in gene_list:
        handle = Entrez.esearch(db="nucleotide", term=gene)
        record = Entrez.read(handle)
        for uid in record["IdList"]:
            seq_handle = Entrez.efetch(db="nucleotide", id=uid, rettype="fasta")
            results.append(seq_handle.read())
    return results
```

以下の3点を確認せよ。

1. `Entrez.email` の設定は行われているか
2. レート制限（1秒あたり3リクエスト）は遵守されているか
3. エラーハンドリングは適切か

（ヒント）NCBIは `Entrez.email` が未設定のリクエストをブロックする可能性がある。また、ループ内で連続してAPIを呼び出しており、レート制限に抵触しうる。

### 演習 19-3: データ取得戦略の選択 **[設計判断]**

1,000件の遺伝子のUniProt機能アノテーションを取得したい。以下の4つの戦略を比較し、状況に応じた最適な選択を判断せよ。

1. REST APIで1件ずつ取得
2. UniProtのIDマッピングサービスで一括取得
3. UniProtの全データをダウンロードしてローカルで検索
4. クラウド環境（AWS/GCP）上で`aws s3 cp`等を使いS3パブリックバケットから直接取得

（ヒント）件数、ネットワーク環境、再利用頻度、レート制限を判断基準にする。1,000件程度であればIDマッピングの一括取得が効率的だが、繰り返し利用するならローカルDBの構築も検討に値する。クラウド上で解析する場合は、データ転送コストとリージョンの一致も考慮せよ。

### 演習 19-4: チェックサム検証 **[実践]**

エージェントに以下を指示し、生成されたコードをレビューせよ。

1. NCBIからリファレンスゲノム（例: GRCh38）をダウンロードするスクリプトを生成させる
2. ダウンロードしたファイルのMD5チェックサムを計算し、NCBI公開のMD5値と比較する検証コードを含めさせる

レビュー時には、`hashlib.md5()` の使い方、大容量ファイルのチャンク読み込み、検証失敗時のエラー処理が適切かを確認せよ。

（ヒント）`hashlib.md5()` でハッシュを計算し、NCBIが公開しているMD5チェックサムファイルの値と比較する。大容量ファイルは一括読み込みではなくチャンク単位で処理する必要がある。
...
- **Sayers, E. W. et al. "Database resources of the National Center for Biotechnology Information". *Nucleic Acids Research*, 52(D1), D33–D43, 2024.** https://pubmed.ncbi.nlm.nih.gov/37994677/ — NCBIのデータベースリソース全体を概説する年次レビュー。毎年更新されるため、最新の機能やデータベースの変更を把握できる。
- **Manzoni, C. et al. "Genome, transcriptome and proteome: the rise of omics data and their integration in biomedical sciences". *Briefings in Bioinformatics*, 19(2), 286–302, 2018.** https://pubmed.ncbi.nlm.nih.gov/27881428/ — オミクスデータとデータベースの全体像を俯瞰するレビュー。各データベースの位置づけと統合の方向性を理解できる。
...
[5] Kanehisa, M. & Goto, S. "KEGG: Kyoto Encyclopedia of Genes and Genomes". *Nucleic Acids Research*, 28(1), 27-30, 2000. https://pubmed.ncbi.nlm.nih.gov/10592173/

[6] Sayers, E. W. et al. "Database resources of the National Center for Biotechnology Information". *Nucleic Acids Research*, 52(D1), D33-D43, 2024. https://pubmed.ncbi.nlm.nih.gov/37994677/

[7] The UniProt Consortium. "UniProt: the Universal Protein Knowledgebase in 2023". *Nucleic Acids Research*, 51(D1), D523-D531, 2023. https://pubmed.ncbi.nlm.nih.gov/36408920/
...
[13] Martin, F. J. et al. "Ensembl 2023". *Nucleic Acids Research*, 51(D1), D933-D941, 2023. https://pubmed.ncbi.nlm.nih.gov/36318249/

[14] Nassar, L. R. et al. "The UCSC Genome Browser database: 2023 update". *Nucleic Acids Research*, 51(D1), D1188-D1195, 2023. https://pubmed.ncbi.nlm.nih.gov/36420891/
