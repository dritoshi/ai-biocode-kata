...
1,000件の遺伝子のUniProt機能アノテーションを取得したい。以下の3つの戦略を比較し、状況に応じた最適な選択を判断せよ。

1. REST APIで1件ずつ取得
2. UniProtのIDマッピングサービスで一括取得
3. UniProtの全データをダウンロードしてローカルで検索

（ヒント）件数、ネットワーク環境、再利用頻度、レート制限を判断基準にする。1,000件程度であればIDマッピングの一括取得が効率的だが、繰り返し利用するならローカルDBの構築も検討に値する。
