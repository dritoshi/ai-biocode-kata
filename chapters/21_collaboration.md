...
本章では、まず[20-1節](#21-1-質問の技術--正しく助けを求める)で質問の構成と質問先の使い分けを学ぶ。次に[20-2節](#21-2-コードレビューとossコミュニティへの参加)でコードレビューの作法とOSSコミュニティへの参加方法を扱う。最後に[20-3節](#21-3-共同研究のコミュニケーション--piと共同研究者への共有)で共同研究者やPIへの進捗共有と解析依頼の受け取り方を解説する。
...
**最小再現例**（Minimal Reproducible Example; MRE）とは、問題を再現するために必要な最小限のコードである。[§9 デバッグ](./09_debug.md)で学んだ「問題の局所化」技法の応用であり、自分のプロジェクト固有のコードを削ぎ落として、問題の本質だけを残す作業である。
...
[§7-2 GitHubの活用](./07_git.md#7-2-githubの活用)で学んだIssueの作成は、特定のツールやライブラリのバグ報告に適している。バグ報告Issueの構造は20-1-1節の質問構成と同じだが、加えて以下を含める:
...
[§6 開発環境](./06_dev_environment.md)で学んだcondaのバイオインフォチャンネルである**bioconda**[4](https://bioconda.github.io/contributor/index.html)は、コミュニティドリブンで運営されている。新しいツールのレシピ（パッケージ定義ファイル）を追加するPRや、既存ツールのバージョン更新PRは、初心者でも取り組みやすい貢献形態である。
...
[§18 ドキュメント化](./18_documentation.md)で学んだCHANGELOG管理の応用として、git logから「先週やったこと」を自動的に要約するアプローチがある。
...
これらの記録には、[§18 ドキュメント化](./18_documentation.md)で学んだANALYSIS_LOG.mdが役立つ。

#### 生物学的解釈の添え方

解析結果を報告する際は、数値だけでなく**生物学的文脈**を添える。[§13 可視化](./13_visualization.md)で学んだ図表作成技術を活用して、視覚的に分かりやすく伝える。
...
> - カラーバーや凡例を必ず付ける。色覚多様性に配慮したカラーパレットを使う（[§13 可視化](./13_visualization.md)参照）
>
