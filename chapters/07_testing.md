...
> 「`scripts/ch07/seq_stats.py` の現在の動作を保証するテストをまず書いてください。テストが通ることを確認してから、リファクタリングに進んでください」
...
> 「`scripts/ch07/seq_stats.py` の全関数に対するpytestのテストを書いてください。正常系に加えて、空入力・境界値・不正入力のエッジケースも網羅してください。Arrange-Act-Assertパターンに従い、各テストにdocstringを付けてください」
...
> 「`scripts/ch07/` の全関数に型ヒントを追加してください。引数と戻り値の型を推測して付けてください。追加後に `mypy scripts/ch07/` を実行して型エラーがないことを確認してください」
