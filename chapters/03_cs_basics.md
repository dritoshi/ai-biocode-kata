...
§3-5で「静的型付けvs動的型付け」の違いを学んだ。Pythonは動的型付け言語だが、**型ヒント**（type hint）を使うことで、静的解析の恩恵を享受できる。型ヒントとは、変数や関数の引数・戻り値に「期待する型」を注釈として記述する仕組みである。Pythonの実行時には型ヒントは無視されるが、mypyなどの静的解析ツール（[§7-3](./07_testing.md)で扱う）がコードを実行前にチェックし、型の不整合を検出できる。
...
型ヒントについてさらに詳しくは、mypyによる静的型チェックの実践を [§7-3 コード品質ツール](./07_testing.md)で扱う。
