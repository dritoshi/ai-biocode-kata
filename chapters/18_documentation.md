...
> （ドキュメントとは、未来の自分に宛てたラブレターである。）
> — Damian Conway, *Perl Best Practices* (O'Reilly, 2005), p.153
...
> <a id="column-ch18-01"></a> **🧬 コラム: 解析パイプラインをMermaidで図示する**
>
...
[§1-1 設計原則とプログラミングパラダイム](./01_design.md#設計原則とプログラミングパラダイム)で扱った命令型・手続き型、オブジェクト指向、関数型、宣言型が主に計算やプログラム構造の捉え方を表すのに対し、文芸的プログラミングはコードを人間へどのような順序で説明するかに着目する。分類の軸が異なるため、たとえば関数型のコードを文芸的プログラミングの形式で記述することもできる。
...
MkDocs[6](https://www.mkdocs.org/) は、Markdownファイルからスタティックサイトを生成するツールである。スタティックサイトとは、あらかじめ生成した HTML / CSS / JavaScript をそのまま配信するサイトであり、アクセスのたびにサーバ側でページを組み立てる動的Webアプリとは対照的である。ドキュメント用途では、更新時にまとめて生成できれば十分なことが多く、Git で管理しやすく、GitHub Pages などにもそのまま配置しやすい。mkdocstrings[7](https://mkdocstrings.github.io/) プラグインを組み合わせることで、Pythonソースコードのdocstringを自動的に取り込んだAPIリファレンスを生成できる。
...
> <a id="column-ch18-02"></a> **🤖 コラム: MLプロジェクトのModel Card**
>
...
[18-1節](#18-1-markdownの習得)で紹介したQuarto[4](https://quarto.org/)は、`.qmd` ファイル（プレーンテキストのMarkdown）にコードブロックを含める形式である。`.ipynb` と異なりJSON構造を持たないため、`git diff` で差分が明瞭に表示される。
...
> <a id="column-ch18-03"></a> **🧬 コラム: MIAME/MINSEQE — 実験の最小記録要件**
>
...
### 演習 18-1: READMEの品質評価 **[レビュー]**

以下のREADMEを読み、不足している要素をすべて指摘せよ。

```markdown
# vcf-filter

## Installation
pip install vcf-filter
```

（ヒント）README に必要な要素を本章のチェックリストと照らし合わせよ。目的・概要、依存関係、使用例、ライセンス、引用方法などが欠けている。

### 演習 18-2: docstring生成の指示 **[指示設計]**

既存の関数（例: VCFファイルのフィルタリング関数）にNumPyスタイルのdocstringを追加させるエージェントへの指示文を書け。指示には以下のセクションを含めるよう明示すること。
...
（ヒント）「NumPyスタイルのdocstring」と具体的に指定すると、エージェントが正しいフォーマットを使う。Googleスタイルとの違いを意識して、明示的にスタイルを指定することが重要である。

### 演習 18-3: Notebook vs Quartoの選択 **[設計判断]**

研究ノートとして解析手順を記録したい。Jupyter Notebook（`.ipynb`）とQuarto（`.qmd`）を以下の2軸で比較し、どちらを採用するか判断せよ。

1. **Git管理の容易さ**: 差分の可読性、マージコンフリクトの扱いやすさ
2. **共有性**: 共同研究者への共有、再現性の確保

（ヒント）Notebookのセル出力はバイナリに近く、`git diff` が困難である。nbstripoutで出力を自動除去する運用か、Quarto（`.qmd`）でプレーンテキスト管理するかの二択を検討せよ。

### 演習 18-4: ドキュメント自動生成 **[実践]**

エージェントに以下を指示し、docstringからAPIドキュメントを自動生成する環境を構築せよ。

1. MkDocsの設定ファイル（`mkdocs.yml`）を作成させる
2. mkdocstringsプラグインを導入し、docstringからAPIリファレンスを自動生成させる
3. `mkdocs serve` でローカルプレビューが動作することを確認する

（ヒント）mkdocs-materialテーマとmkdocstringsプラグインの組み合わせが標準的である。エージェントに「mkdocs-materialテーマを使い、mkdocstringsでPythonのdocstringからAPIドキュメントを生成する設定を作って」と指示するとよい。

---

## さらに学びたい読者へ

本章で扱ったドキュメント化の手法と思想をさらに深く学びたい読者に向けて、原典と実践的なフレームワークを紹介する。
...
- **Wilson, G. et al. "Good Enough Practices in Scientific Computing". *PLOS Computational Biology*, 13(6), e1005510, 2017.** https://doi.org/10.1371/journal.pcbi.1005510 — READMEの書き方、データドキュメントの作法を含む「十分な」ドキュメント実践を提唱した論文。
- **Rule, A. et al. "Ten Simple Rules for Writing and Sharing Computational Analyses in Jupyter Notebooks". *PLOS Computational Biology*, 15(7), e1007007, 2019.** https://doi.org/10.1371/journal.pcbi.1007007 — ノートブック形式のドキュメントのベストプラクティスを10のルールで整理。
