...
- **Wilson, G. et al. "Good Enough Practices in Scientific Computing". *PLOS Computational Biology*, 13(6), e1005510, 2017.** — READMEの書き方、データドキュメントの作法を含む「十分な」ドキュメント実践を提唱した論文。
- **Rule, A. et al. "Ten Simple Rules for Writing and Sharing Computational Analyses in Jupyter Notebooks". *PLOS Computational Biology*, 15(7), e1007007, 2019.** — ノートブック形式のドキュメントのベストプラクティスを10のルールで整理。
