...
前章の[§18 データベースとAPI](./18_database_api.md)では、公開データベースからのデータ取得とAPIキーの管理を学んだ。本章では、そうしたデータや認証情報を**安全に**扱うための体系的な知識を整理する。
...
本章では、まず[19-1節](#19-1-セキュリティの基礎)でシークレット管理・エージェントのセキュリティ設定・依存パッケージの安全性を扱い、次に[19-2節](#19-2-データの倫理)でヒトデータの法規制・匿名化・研究倫理を学ぶ。
...
APIキー、データベースパスワード、SSH秘密鍵——これらの**シークレット**（秘密情報）は、バイオインフォマティクスのワークフローに不可欠だが、取り扱いを一歩間違えると深刻なセキュリティ事故につながる。[§8 環境変数による秘密情報の管理](./08_deliverables.md#環境変数による秘密情報の管理)では`os.environ.get()`による環境変数からの読み取りを紹介したが、シークレット管理にはライフサイクル全体を見渡す視点が必要である。
...
3. **利用**: 環境変数経由でプログラムに渡す（[§8](./08_deliverables.md#環境変数による秘密情報の管理)で学んだパターン）。ログやエラーメッセージにシークレットが出力されないよう注意する
...
[§6 .gitignore](./06_git.md#gitignore--管理すべきでないファイルの除外)で学んだように、`.env`ファイルは必ず`.gitignore`に記載してバージョン管理から除外する。開発環境では`python-dotenv`ライブラリで`.env`を読み込むのが標準的なパターンである。
...
`git secrets --install`は`.git/hooks/`にpre-commitフックを追加する。以降、パターンにマッチする行を含むファイルをコミットしようとするとエラーになる。[§7 テスト](./07_testing.md)で学んだpre-commitフレームワークと組み合わせれば、リンター・フォーマッタ・シークレットスキャンを一つのフックチェーンで実行できる。

以下は、git-secretsの簡易版としてPythonで実装したシークレットスキャナである。正規表現でコードベース内のAPIキーやパスワードのパターンを検出する。

```python
# scripts/ch19/secret_scanner.py — シークレットスキャナの中核部分
...
`scan_content`関数は各行を全パターンと照合し、マッチした行の情報を`SecretFinding`データクラスとして返す。完全なコードは`scripts/ch19/secret_scanner.py`にあり、ディレクトリ全体の再帰スキャン機能も含む。
...
### 19-1-2. AIエージェントのセキュリティとハーネス

エージェントは強力なツールだが、制限なく動作させることにはリスクが伴う。[§0 承認モード](./00_ai_agent.md#承認モード--3段階の安全レベル)で3段階の権限レベル（読み取り専用・承認あり・全自動）を学んだ。本節ではその背景にあるセキュリティの原則を掘り下げる。

#### 最小権限の原則

**最小権限の原則**（Principle of Least Privilege）とは、タスクの遂行に必要な最小限の権限のみを付与する設計原則である。[§13 セキュリティ制約](./13_container.md#セキュリティ制約)ではDockerコンテナの権限制御を学んだが、同じ原則がエージェントにも適用される。
...
|-------|------|--------------------------|
| **個人情報保護法**[11](https://www.ppc.go.jp/personalinfo/legal/gentec_data_guideline/) | 日本 | ゲノムデータ（塩基配列）は「個人識別符号」として個人情報に該当。疾患関連の解釈を含む場合は「要配慮個人情報」としてより厳格な同意が必要。3年ごと見直し改正が2027年施行見込み |
| **次世代医療基盤法**[12](https://www8.cao.go.jp/iryou/kouhou/pdf/kaisei_jisedaiiryou_rikatsuyou.pdf) | 日本 | 2024年改正で「仮名加工医療情報」を新設。認定事業者に限定された利活用制度 |
| **GDPR** | EU | 遺伝データは「特別カテゴリーの個人データ」。2025年、欧州データ保護委員会（EDPB）がAIモデルへの個人データ適用に関するガイダンスを公表。EU AI Act[15](https://artificialintelligenceact.eu/)（2024年発効）では科学研究目的のAIは基本的に適用除外だが、医療AI機器は高リスク分類の対象 |
| **HIPAA**[14](https://www.federalregister.gov/documents/2025/01/06/2024-30983/hipaa-security-rule-to-strengthen-the-cybersecurity-of-electronic-protected-health-information) | 米国 | 医療情報の保護。18種類の識別子の除去が「非識別化」の基準。2025年改正案でAI利用時のリスク分析・多要素認証（MFA）が義務化の方向。AIツールでのPHI処理は治療・支払・医療業務（TPO）に該当しない場合、個別の患者同意が必要になりうる |
...
| gpt-oss | 20B | 16GB | OpenAI製オープンモデル（Apache 2.0）。SWE-bench高スコア |
...
```python
# scripts/ch19/anonymize_metadata.py — 年齢の一般化
...
```python
# scripts/ch19/anonymize_metadata.py — k-匿名性検証
...
完全なコードは`scripts/ch19/anonymize_metadata.py`にあり、CSV全体の匿名化パイプラインとk-匿名性検証を含む。

**合成データの活用**: テストやデバッグには本物の患者データを使わず、合成データ（synthetic data）を生成して使う。[§7 テスト](./07_testing.md)でも述べたように、本番データをリポジトリに含めることは厳禁である。
...
**データ管理計画**（Data Management Plan; DMP）は、研究プロジェクトで生成・取得するデータの管理方針を文書化したものである。2024年度（令和6年度）以降、日本学術振興会（JSPS）の科研費ではDMPの作成が事実上必須となった。2025年度からは実績報告書・成果報告書の提出時にDMPに基づく研究データのメタデータ報告が義務化されている[13](https://www.jsps.go.jp/j-grantsinaid/01_seido/10_datamanagement/index.html)。
...
研究データの公開・共有を促進するオープンサイエンスの理念と、個人情報の保護は時に緊張関係にある。[§18 FAIR原則](./18_database_api.md)で学んだように、データのFindable（発見可能）、Accessible（アクセス可能）、Interoperable（相互運用可能）、Reusable（再利用可能）を実現することは重要だが、ヒトデータについてはAccessibleの部分に制限を設けざるを得ない。
...
> 「科研費のデータ管理計画（DMP）のテンプレートを作成して。RNA-seq解析プロジェクトを想定して、データの種類、保存場所、保存期間、アクセス制御、共有方法、破棄手順の各項目を埋めて」
...
| **ローカルLLMエージェント** | 制限付きデータにはローカル実行が必須。Ollama + Qwen2.5-Coder / gpt-oss等で完全オフライン環境を構築 |
...
次章の[§20 コラボレーション](./20_collaboration.md)では、質問の技術、コードレビューとOSSコミュニティへの参加、共同研究者やPIへの進捗共有など、他の研究者やエンジニアとの協働を学ぶ。
...
[15] European Parliament. "EU Artificial Intelligence Act". https://artificialintelligenceact.eu/ (参照日: 2026-03-22)
