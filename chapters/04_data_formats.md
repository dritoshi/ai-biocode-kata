...
> **この章の前提知識**: DNA・RNA・タンパク質が何かを知っていること。次世代シーケンサーの出力がFASTQであることを概念レベルで知っていること。不安な場合は [統合TV](https://togotv.dbcls.jp/) や坊農秀雅『Dr. Bonoの生命科学データ解析』を参照。
...
> <a id="column-ch04-01"></a> **🧬 コラム: Excelと遺伝子名 — Oct4問題**
>
...
> <a id="column-ch04-02"></a> #### 🧬 コラム: 日本語環境の罠 — Shift_JIS, BOM, NFD, マルチバイト文字
>
...
>
> - `int()` は全角数字も全角スペースも**黙って受理する** → `int("１２３")` は `123`、`int("　100")` は `100` を返す。例外が出ないまま、混入した全角文字が数値に化けて通ってしまう
> - 引数なしの `str.split()` は全角スペース(U+3000)も区切り文字として扱う → 空白区切りを想定した処理では余分な列ができ、カラムがずれる。一方 `split("\t")` や `awk` は全角スペースで分割しないため、今度は値の中に紛れ込んだまま残る
> - FASTAヘッダーに全角文字が入ると、BLASTなどのツールが予期しない挙動を示す
>
> 最大の問題は、**多くの場合エラーにならず、静かに壊れる**ことである。`int()` は全角混じりの値も黙って数値へ変換し、`split("\t")` は全角スペースを値の中に残す。どちらも例外を出さないため、カラムがずれたり値が変わったまま下流の解析が進み、結果がおかしいと気づくまで原因がわからない。
>
> ```python
> # 全角スペースが混入したTSV行
> line = "gene1\t\u3000100\t200"  # 見た目は正常だが…
> fields = line.split("\t")
> print(fields)         # ['gene1', '\u3000100', '200'] — 全角スペースが値に残る
> print(int(fields[1])) # 100 — 例外は出ない。全角スペースが黙って除かれる
> print(int("１２３"))    # 123 — 全角数字もそのまま数値になる
> ```
>
...
> <a id="column-ch04-03"></a> **🧬 コラム: バイオインフォの主要フォーマット**
>
...
> <a id="column-ch04-04"></a> **🧬 コラム: 座標系の罠 — 0-based vs 1-based**
>
...
>
> BED↔GFF座標変換の具体的な実装は [`scripts/ch04/coordinate_convert.py`](https://github.com/dritoshi/ai-biocode-kata/blob/main/scripts/ch04/coordinate_convert.py) を参照。

> <a id="column-ch04-05"></a> 🧬 **コラム: 遺伝子構造の基礎知識 — フォーマットの理解に必要な生物学**
>
...
> <a id="column-ch04-06"></a> 🧬 **コラム: リファレンスゲノムの選び方**
>
...
> <a id="column-ch04-07"></a> **🤖 コラム: 機械学習・数値計算のデータフォーマット**
>
...
（ヒント）精度保持（浮動小数点の丸め）、ファイルサイズ、下流ツール（DESeq2、pandas、scanpy等）の入力形式を考慮する。「中間データ」であることの意味も判断材料になる。さらに、このデータを論文投稿時に公開する場合、どのライセンス（CC-BY等）でどのリポジトリ（Zenodo等）に登録するかも検討せよ。
