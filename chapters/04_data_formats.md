...
NCBI E-utilities、Ensembl REST API、UniProt APIなど、主要なバイオデータベースのAPIはJSONでデータを返す。[§19 データベースとAPI](./19_database_api.md)で詳しく扱う。
...
次章の[§5 ソフトウェアの構成要素](./05_software_components.md)では、「ライブラリ」「パッケージ」「モジュール」「API」といった用語の正確な意味と、`import` 文の裏で何が起きているかを学ぶ。
