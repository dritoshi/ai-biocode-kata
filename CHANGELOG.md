...
- 本文コードと `scripts/`・`tests/` の対応関係を分類・検証するレビュー基盤を追加
- 配置が必要なコードサンプル、設定例、ワークフロー定義と回帰テストを追加
- Snakemake、Nextflow、CWL、Podman、MkDocs等をApple Silicon上で確認する外部ランタイム監査を追加
...
- 全22章の事実関係、引用、URL、ツール仕様、法制度、データベース情報を2026年7月時点へ更新
- Claude Code CLIとCodex CLIのモデル・推論設定をClaude Opus 5、GPT-5.6世代まで更新
- 本文中の実装コードと `scripts/` の差をE5・E3・E1の順に監査し、E5・E3を解消して、E1の意図的な説明差には教育的コメントを追加
- Pythonサンプルの依存関係を `pyproject.toml` と `uv.lock` に集約し、CIをuvベースへ移行
- 各章の文字数と読了時間を全31原稿から再集計

### Fixed

- 本文に必要な実体がないE5と、振る舞いが異なるE3をすべて解消し、E1を教育上妥当な差として確認
- 引用の書誌情報、未使用参考文献、リンクされていない引用番号、章間参照を全章で再監査
- 確定したURLの404・HTTPエラーを解消し、到達性を自動・手動で再確認
- EPUB内のコード参照をビルド元コミットのGitHub恒久リンクへ変換
- PDF・EPUBでコラム索引と本文中のコラム参照を移動可能なリンクとして出力
- GitHub Actions、ruff、pytest、外部ランタイムテストの不整合を修正
